...
ID3D11VertexShader*					g_pPatchSkinningVS = NULL;
ID3D11VertexShader*					g_pMeshSkinningVS = NULL;
ID3D11HullShader*					g_pSubDToBezierHS = NULL;
ID3D11HullShader*					g_pSubDToBezierHS4444 = NULL;
        ...
        if( strCommand[0] == L'-' || strCommand[0] == L'/' )
        {
            ...
            else if( wcsstr( strCommand, L"msaa" ) == strCommand )
            {
                ...
                if( strCommand != NULL )
                {
                    ...
                    g_iMSAASampleCount = min( 16, max( 1, g_iMSAASampleCount ) );
                }
            }
            ...
            else if( wcsstr( strCommand, L"cameraname" ) == strCommand )
            {
                ...
                if( strCommand != NULL )
                {
                    ...
                    WideCharToMultiByte( CP_ACP, 0, strName, (int)wcslen( strName ), g_strCameraName, ARRAYSIZE(g_strCameraName), NULL, NULL );
                }
            }
            ...
            else if( wcsstr( strCommand, L"subdiv" ) == strCommand )
            {
                ...
                if( strCommand != NULL )
                {
                    ...
                    g_iSubdivs = min( 15, max( 1, g_iSubdivs ) );
                }
            }
        }

        if( wcsstr( strCommand, L".sdkmesh" ) != NULL )
        {
...
//--------------------------------------------------------------------------------------
// Use D3DX11 compilation helper to compile the shader
//--------------------------------------------------------------------------------------
                                      ...
                                      void* pUserContext )
{
    ...
    // Compile shaders
    ID3DBlob* pBlobPatchVS = NULL;
    ID3DBlob* pBlobMeshVS = NULL;
    ID3DBlob* pBlobHS = NULL;
    ID3DBlob* pBlobHS4444 = NULL;
    ID3DBlob* pBlobDS = NULL;
    ID3DBlob* pBlobPS = NULL;
    ID3DBlob* pBlobPSSolid = NULL;

    V_RETURN( CompileShaderFromFile( L"SubD11.hlsl", "PatchSkinningVS","vs_5_0", &pBlobPatchVS ) );
    V_RETURN( CompileShaderFromFile( L"SubD11.hlsl", "MeshSkinningVS", "vs_5_0", &pBlobMeshVS ) );
    V_RETURN( CompileShaderFromFile( L"SubD11.hlsl", "SubDToBezierHS", "hs_5_0", &pBlobHS ) );
    V_RETURN( CompileShaderFromFile( L"SubD11.hlsl", "SubDToBezierHS4444", "hs_5_0", &pBlobHS4444 ) );
    ...
    // Create shaders
    V_RETURN( pd3dDevice->CreateVertexShader( pBlobPatchVS->GetBufferPointer(), pBlobPatchVS->GetBufferSize(), NULL, &g_pPatchSkinningVS ) );
    DXUT_SetDebugName( g_pPatchSkinningVS, "PatchSkinningVS" );

    V_RETURN( pd3dDevice->CreateVertexShader( pBlobMeshVS->GetBufferPointer(), pBlobMeshVS->GetBufferSize(), NULL, &g_pMeshSkinningVS ) );
    ...
    V_RETURN( pd3dDevice->CreateHullShader( pBlobHS4444->GetBufferPointer(), pBlobHS4444->GetBufferSize(), NULL, &g_pSubDToBezierHS4444 ) );
    ...
    if( g_bMovieMode )
    {
        ...
        else
        {
            ...
            g_fMovieEndTime = min( g_fMovieEndTime, fAnimDuration );
            g_fMovieStartTime = min( g_fMovieStartTime, g_fMovieEndTime );
    ...
    else
    {
        vEye = vCenter;
        FLOAT fRadius = D3DXVec3Length( &vExtents );
        const FLOAT fTheta = D3DX_PI * 0.125f;
                                          ...
                                          const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc, void* pUserContext )
{
    ...
    FLOAT fFOV = g_fFieldOfView * ( D3DX_PI / 180.0f );
...
void RenderSubDMesh( ID3D11DeviceContext* pd3dDeviceContext, CSubDMesh* pMesh, ID3D11PixelShader* pPixelShader )
{
    ...
    // Set the shaders
    pd3dDeviceContext->VSSetShader( g_pPatchSkinningVS, NULL, 0 );
    ...
    pd3dDeviceContext->HSSetShader( g_pSubDToBezierHS4444, NULL, 0 );
    for( int i = 0; i < PieceCount; ++i )
    {
        ...
        for( int j = 0; j < NumTransforms; ++j )
        {
            ...
            {
                D3DXMatrixIdentity( &pData->mConstBoneWorld[j] );
            }
            else
            {
                D3DXMatrixTranspose( &pData->mConstBoneWorld[j], pMesh->GetInfluenceMatrix( MeshIndex, j ) );
            }
        }
        if( NumTransforms == 0 )
        {
            D3DXMATRIX matTransform;
            pMesh->GetPatchPieceTransform( i, &matTransform );
    ...
    pd3dDeviceContext->VSSetShader( g_pMeshSkinningVS, NULL, 0 );
    ...
    for( INT i = 0; i < PieceCount; ++i )
    {
        ...
        {
            D3DXMATRIX matTransform;
            pMesh->GetPolyMeshPieceTransform( i, &matTransform );
            D3DXMatrixTranspose( &pData->mConstBoneWorld[0], &matTransform );
        }
        pd3dDeviceContext->Unmap( g_pcbPerMesh, 0 );
        pd3dDeviceContext->VSSetConstantBuffers( g_iBindPerMesh, 1, &g_pcbPerMesh );
                                  ...
                                  float fElapsedTime, void* pUserContext )
{
    ...
    D3DXVECTOR3 vCameraPosWorld = *( g_Camera.GetEyePt() );
    ...
    pData->vCameraPosWorld = vCameraPosWorld;
    ...
    if( g_bDrawWires )
    {
        ...
        pData->fTessellationFactor = (float)g_iSubdivs;
        pData->fDisplacementHeight = g_fDisplacementHeight;
        pData->vSolidColor = D3DXVECTOR3( 0, 1, 0 );
        pd3dImmediateContext->Unmap( g_pcbPerFrame, 0 );
    ...
    if( g_bMovieMode )
    {
        ...
        DXUTSnapD3D11Screenshot( strCapName, D3DX11_IFF_BMP );
