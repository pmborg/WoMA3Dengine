...
#define ADD_STRING_LINE_MACRO(x,y)     { AddString( hwndList, x, TEXT( #y ), y ); nElementCount++; }
#define ADD_EXPANDED_STRING_LINE_MACRO(x,y)  { AddExpandedString( hwndList, x, TEXT( #y ), y ); nElementCount++; }
#define ADD_INT_LINE_MACRO(x,y)        { AddString( hwndList, x, TEXT( #y ), _ltot(y,szTmp,10) ); nElementCount++; }
#define ADD_UINT_LINE_MACRO(x,y)       { AddString( hwndList, x, TEXT( #y ), _ultot(y,szTmp,10) ); nElementCount++; }
#define ADD_INT64_LINE_MACRO(x,y)      { AddString( hwndList, x, TEXT( #y ), _ui64tot(y,szTmp,10) ); nElementCount++; }
#define ADD_UINT_LINE_MACRO2(x,y,z)       { AddString( hwndList, x, y, _ultot(z,szTmp,10) ); nElementCount++; }


//-----------------------------------------------------------------------------
// Global variables
//-----------------------------------------------------------------------------
CDxDiagInfo*    g_pDxDiagInfo = NULL;
...
VOID FillListBoxWithInputRelatedInfo( HWND hDlg, vector <InputRelatedDeviceInfo*>& vDeviceList );
                    ...
                    LPSTR lpCmdLine, INT nCmdShow )
{
    ...
    if( g_pDxDiagInfo == NULL )
        return 0;

    if( FAILED( hr = g_pDxDiagInfo->Init( TRUE ) ) )
    {
        delete g_pDxDiagInfo;
        MessageBox( NULL, TEXT( "Failed initializing dxdiagn.dll" ), TEXT( "Error" ), MB_OK );
        return 0;
    }
    ...
    {
        delete g_pDxDiagInfo;
        MessageBox( NULL, TEXT( "Failed querying dxdiagn.dll" ), TEXT( "Error" ), MB_OK );
        return 0;
    }
...
VOID FillListBoxWithSysInfo( HWND hwndList )
{
    ...
    TCHAR szTmp[3000];
    TCHAR szKey[] = TEXT( "DxDiag_SystemInfo" );
    ...
    {
        TCHAR szName[256];
        StringCchPrintf( szName, 256, TEXT( "pSysInfo->m_ExtFuncBitmasks[%d].dwBits0_31" ), i );
        ADD_UINT_LINE_MACRO2( szKey, szName, pSysInfo->m_ExtFuncBitmasks[i].dwBits0_31 );
        StringCchPrintf( szName, 256, TEXT( "pSysInfo->m_ExtFuncBitmasks[%d].dwBits32_63" ), i );
        ADD_UINT_LINE_MACRO2( szKey, szName, pSysInfo->m_ExtFuncBitmasks[i].dwBits32_63 );
        StringCchPrintf( szName, 256, TEXT( "pSysInfo->m_ExtFuncBitmasks[%d].dwBits64_95" ), i );
        ADD_UINT_LINE_MACRO2( szKey, szName, pSysInfo->m_ExtFuncBitmasks[i].dwBits64_95 );
        StringCchPrintf( szName, 256, TEXT( "pSysInfo->m_ExtFuncBitmasks[%d].dwBits96_127" ), i );
...
VOID FillListBoxWithLogicalDiskInfo( HWND hwndList )
{
    ...
    vector <LogicalDisk*>::iterator iterDisk;
    for( iterDisk = pDxDiag->m_vLogicalDiskList.begin(); iterDisk != pDxDiag->m_vLogicalDiskList.end(); iterDisk++ )
    {
        pLogicalDisk = *iterDisk;
        _tcsncpy( szName, pLogicalDisk->m_szDriveLetter, 30 );
        szName[29] = 0;
        ...
        for( iter = pLogicalDisk->m_vDriverList.begin(); iter != pLogicalDisk->m_vDriverList.end(); iter++ )
        {
            pFileNode = *iter;
            _tcsncpy( szName, pLogicalDisk->m_szDriveLetter, 30 );
            szName[29] = 0;
            StringCchCat( szName, 3000, TEXT( ": " ) );
            StringCchCat( szName, 3000, pFileNode->m_szName );
...
VOID FillListBoxWithSystemDevices( HWND hwndList )
{
    ...
    vector <SystemDevice*>::iterator iterDevice;
    for( iterDevice = pDxDiag->m_vSystemDevices.begin(); iterDevice != pDxDiag->m_vSystemDevices.end(); iterDevice++ )
    {
        pSystemDevice = *iterDevice;
        _tcsncpy( szName, pSystemDevice->m_szDescription, 30 );
        szName[29] = 0;
        ...
        FileNode* pFileNode;
        vector <FileNode*>::iterator iter;
        for( iter = pSystemDevice->m_vDriverList.begin(); iter != pSystemDevice->m_vDriverList.end(); iter++ )
        {
            pFileNode = *iter;
            _tcsncpy( szName, pSystemDevice->m_szDescription, 30 );
            szName[29] = 0;
...
VOID FillListBoxWithDirectXFilesInfo( HWND hwndList )
{
    ...
    TCHAR szKey[] = TEXT( "DxDiag_FileInfo" );
    DWORD nElementCount = 0;

    if( pDxDiag->m_pFileInfo == NULL )
        return;
    ...
    vector <FileNode*>::iterator iter;
    for( iter = pDxDiag->m_pFileInfo->m_vDxComponentsFiles.begin();
         iter != pDxDiag->m_pFileInfo->m_vDxComponentsFiles.end(); iter++ )
    {
        pFileNode = *iter;
        TCHAR* szName = pFileNode->m_szName;
        nElementCount = 0;
...
VOID FillListBoxWithDisplayInfo( HWND hwndList )
{
    ...
    for( iter = pDxDiag->m_vDisplayInfo.begin(); iter != pDxDiag->m_vDisplayInfo.end(); iter++ )
    {
        pDisplayInfo = *iter;
        TCHAR* szName = pDisplayInfo->m_szDescription;
        DWORD nElementCount = 0;
...
VOID FillListBoxWithDXVAInfo( TCHAR* szParentName, HWND hwndList, vector <DxDiag_DXVA_DeinterlaceCaps*>& vDXVACaps )
{
    ...
    vector <DxDiag_DXVA_DeinterlaceCaps*>::iterator iter;
    for( iter = vDXVACaps.begin(); iter != vDXVACaps.end(); iter++ )
    {
        pDXVANode = *iter;
        StringCchPrintf( szName, 256, TEXT( "%s : DXVA %d" ), szParentName, dwIndex );
        DWORD nElementCount = 0;
...
VOID FillListBoxWithSoundInfo( HWND hwndList )
{
    ...
    for( iter = pDxDiag->m_vSoundInfos.begin(); iter != pDxDiag->m_vSoundInfos.end(); iter++ )
    {
        pSoundInfo = *iter;
        TCHAR* szName = pSoundInfo->m_szDescription;
        DWORD nElementCount = 0;
    ...
    vector <SoundCaptureInfo*>::iterator iterCapture;
    for( iterCapture = pDxDiag->m_vSoundCaptureInfos.begin(); iterCapture != pDxDiag->m_vSoundCaptureInfos.end();
         iterCapture++ )
    {
        pSoundCaptureInfo = *iterCapture;
        TCHAR* szName = pSoundCaptureInfo->m_szDescription;
        DWORD nElementCount = 0;
...
VOID FillListBoxWithMusicInfo( HWND hwndList )
{
    ...
    TCHAR szKey[] = TEXT( "DxDiag_MusicInfo" );
    DWORD nElementCount = 0;
    ...
    for( iter = pDxDiag->m_pMusicInfo->m_vMusicPorts.begin(); iter != pDxDiag->m_pMusicInfo->m_vMusicPorts.end();
         iter++ )
    {
        pMusicPort = *iter;
        TCHAR* szName = pMusicPort->m_szDescription;
        nElementCount = 0;
        ...
        if( nElementCount != pMusicPort->m_nElementCount )
            OutputDebugString( TEXT("**WARNING** -- not all elements from pMusicPort displayed\n") );
#endif
    }
}
...
VOID FillListBoxWithInputInfo( HWND hwndList )
{
    ...
    TCHAR szKey[] = TEXT( "DxDiag_InputInfo" );
    DWORD nElementCount = 0;
    ...
    for( iter = pDxDiag->m_pInputInfo->m_vDirectInputDevices.begin();
         iter != pDxDiag->m_pInputInfo->m_vDirectInputDevices.end(); iter++ )
    {
        pInputDevice = *iter;
        TCHAR* szName = pInputDevice->m_szInstanceName;
        nElementCount = 0;
...
VOID FillListBoxWithInputRelatedInfo( HWND hwndList, vector <InputRelatedDeviceInfo*>& vDeviceList )
{
    ...
    for( iter = vDeviceList.begin(); iter != vDeviceList.end(); iter++ )
    {
        ...
        TCHAR* szName = pInputRelatedDevice->m_szDescription;
        nElementCount = 0;
        ...
        FileNode* pFileNode;
        vector <FileNode*>::iterator iterFile;
        for( iterFile = pInputRelatedDevice->m_vDriverList.begin();
             iterFile != pInputRelatedDevice->m_vDriverList.end(); iterFile++ )
        {
            pFileNode = *iterFile;
            _tcsncpy( szName, pInputRelatedDevice->m_szDescription, 30 );
            szName[29] = 0;
            StringCchCat( szName, MAX_PATH, TEXT( ": " ) );
            StringCchCat( szName, MAX_PATH, pFileNode->m_szName );
...
VOID FillListBoxWithNetworkInfo( HWND hwndList )
{
    ...
    TCHAR szKey[] = TEXT( "DxDiag_NetInfo" );
    DWORD nElementCount = 0;
    ...
    for( iterNetApp = pDxDiag->m_pNetInfo->m_vNetApps.begin(); iterNetApp != pDxDiag->m_pNetInfo->m_vNetApps.end();
         iterNetApp++ )
    {
        pNetApp = *iterNetApp;
        TCHAR* szName = pNetApp->m_szName;
        nElementCount = 0;
    ...
    for( iterNetSP = pDxDiag->m_pNetInfo->m_vNetSPs.begin(); iterNetSP != pDxDiag->m_pNetInfo->m_vNetSPs.end();
         iterNetSP++ )
    {
        pNetSP = *iterNetSP;
        TCHAR* szName = pNetSP->m_szNameEnglish;
        nElementCount = 0;
    ...
    for( iterNetAdapter = pDxDiag->m_pNetInfo->m_vNetAdapters.begin();
         iterNetAdapter != pDxDiag->m_pNetInfo->m_vNetAdapters.end(); iterNetAdapter++ )
    {
        pNetAdapter = *iterNetAdapter;
        TCHAR szName[200];
        _tcsncpy( szName, pNetAdapter->m_szAdapterName, 50 );
        szName[49] = 0;
    ...
    for( iterNetCodec = pDxDiag->m_pNetInfo->m_vNetVoiceCodecs.begin();
         iterNetCodec != pDxDiag->m_pNetInfo->m_vNetVoiceCodecs.end(); iterNetCodec++ )
    {
        pNetVoiceCodec = *iterNetCodec;
        TCHAR* szName = pNetVoiceCodec->m_szName;
        nElementCount = 0;
...
VOID FillListBoxWithDirectShowInfo( HWND hwndList )
{
    ...
    if( pDxDiag->m_pShowInfo == NULL )
        return;

    ShowFilterInfo* pShowFilterInfo;
    vector <ShowFilterInfo*>::iterator iter;
    for( iter = pDxDiag->m_pShowInfo->m_vShowFilters.begin(); iter != pDxDiag->m_pShowInfo->m_vShowFilters.end();
         iter++ )
    {
        pShowFilterInfo = *iter;
        TCHAR* szName = pShowFilterInfo->m_szName;
        DWORD nElementCount = 0;
...
VOID AddExpandedString( HWND hwndList, TCHAR* szKey, TCHAR* szName, TCHAR* szValue )
{
    TCHAR szBuffer[1024*7];
    _tcsncpy( szBuffer, szValue, 1024 * 7 );
    szBuffer[1024 * 7 - 1] = 0;

    TCHAR* pEndOfLine;
    TCHAR* pCurrent = szBuffer;
    TCHAR* pStartOfNext;
