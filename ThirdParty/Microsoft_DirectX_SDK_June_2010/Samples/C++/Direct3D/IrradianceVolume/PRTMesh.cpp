...
HRESULT CPRTMesh::LoadMesh( IDirect3DDevice9* pd3dDevice, WCHAR* strMeshFileName )
{
    ...
    DWORD* rgdwAdjacency = new DWORD[m_pMesh->GetNumFaces() * 3];
    DWORD* rgdwAdjacencyOut = new DWORD[m_pMesh->GetNumFaces() * 3];
    if( rgdwAdjacency && rgdwAdjacencyOut )
    {
        V( m_pMesh->GenerateAdjacency( 1e-6f, rgdwAdjacency ) );
        V( m_pMesh->Optimize( D3DXMESHOPT_ATTRSORT,
        ...
        V( m_pMesh->Optimize( D3DXMESHOPT_VERTEXCACHE | D3DXMESHOPT_IGNOREVERTS,
                              rgdwAdjacencyOut, NULL, NULL, NULL, &pTempMesh ) );
    }
    SAFE_DELETE_ARRAY( rgdwAdjacency );
    SAFE_DELETE_ARRAY( rgdwAdjacencyOut );
    SAFE_RELEASE( m_pMesh );
    m_pMesh = pTempMesh;
    ...
    for( iPass = 0; iPass < cPasses; iPass++ )
    {
        ...
        for( DWORD i = 0; i < dwAttribs; i++ )
        {
            if( bRenderWithAlbedo )
            {
                ...
                V( m_pSHIrradEnvMapEffect->SetValue( "MaterialDiffuseColor", &m_pMaterials[i].MatD3D.Diffuse, sizeof
                                                     ( D3DCOLORVALUE ) ) );
                                ...
                                bool bUniform, bool bRenderWithAlbedo )
{
    ...
    for( iPass = 0; iPass < cPasses; iPass++ )
    {
        ...
        for( DWORD i = 0; i < dwAttribs; i++ )
        {
            if( bRenderWithAlbedo && m_pAlbedoTextures.GetSize() > ( int )i )
            {
