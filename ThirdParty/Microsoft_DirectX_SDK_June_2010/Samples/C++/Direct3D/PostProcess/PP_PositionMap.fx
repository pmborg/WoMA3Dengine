...
//       a single technique with a pixel shader that outputs the pixel position
//       in view space as its color.
//
...
// Desc: Performs post-processing effect that output the normal as the pixel's
//       color.
//-----------------------------------------------------------------------------
float4 PostProcessPS( float2 Tex : TEXCOORD0 ) : COLOR0
{
    return float4( tex2D( g_samSrcPosition, Tex ).zzz / 3.0f, 1.0f );
}
...
// Desc: Performs post-processing effect.
//-----------------------------------------------------------------------------
technique PostProcess
{
    pass p0
