...
HRESULT COptionsFile::SaveOptions( WCHAR* strFile )
{
    ...
    CXMLHelper::CreateNewValue( pDoc, pTopNode, L"dwRobustMeshRefineMaxSubdiv", m_Options.dwRobustMeshRefineMaxSubdiv );
...
INT_PTR CALLBACK CPRTOptionsDlg::DlgProc( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam )
{
    switch( msg )
    {
        case WM_INITDIALOG:
        {
            ...
            m_hMsgProcHook = SetWindowsHookEx( WH_GETMESSAGE, GetMsgProc, (HINSTANCE)GetModuleHandle(NULL), GetCurrentThreadId() );
            ...
            switch( LOWORD(wParam) )
            {
                ...
                case IDC_INPUT_BROWSE_BUTTON:
                {
                    ...
                    if( TRUE == GetOpenFileName( &ofn ) )
                    {
                        ...
                        SendMessage( GetDlgItem( hDlg, IDC_MESH_NAME ), EM_SETSEL, wcslen(GetGlobalOptions().strInputMesh), wcslen(GetGlobalOptions().strInputMesh) );
                ...
                case IDC_OUTPUT_BROWSE_BUTTON:
                {
                    ...
                    if( bResult )
                    {
                        ...
                        SendMessage( GetDlgItem( hDlg, IDC_OUTPUT_EDIT ), EM_SETSEL, wcslen(strResults), wcslen(strResults) );
                ...
                case IDC_OUTPUT_MESH_BROWSE_BUTTON:
                {
                    ...
                    if( TRUE == GetSaveFileName( &ofn ) )
                    {
                        ...
                        SetCurrentDirectory( GetGlobalOptions().strInitialDir );
                        SetDlgItemText( hDlg, IDC_OUTPUT_MESH_EDIT, GetGlobalOptions().strOutputMesh );
                        SendMessage( GetDlgItem( hDlg, IDC_OUTPUT_MESH_EDIT ), EM_SETSEL, wcslen(GetGlobalOptions().strOutputMesh), wcslen(GetGlobalOptions().strOutputMesh) );
...
*/
                    ...
                    GetGlobalOptions().dwNumBounces = (DWORD) LOWORD( SendMessage( GetDlgItem( hDlg, IDC_NUM_BOUNCES_SPIN ), UDM_GETPOS, 0, 0 ) );
                    GetGlobalOptions().dwNumRays = (DWORD) LOWORD( SendMessage( GetDlgItem( hDlg, IDC_NUM_RAYS_SPIN ), UDM_GETPOS, 0, 0 ) );
                    ...
                    GetGlobalOptions().dwNumChannels = (IsDlgButtonChecked( hDlg, IDC_SPECTRAL_CHECK ) == BST_CHECKED ) ? 3 : 1;
                    ...
                    GetDlgItemText( hDlg, IDC_ABSORPTION_R_EDIT, sz, 256 ); if( swscanf( sz, TEXT("%f"), &GetGlobalOptions().Absorption.r ) == 0 ) return false;
                    GetDlgItemText( hDlg, IDC_ABSORPTION_G_EDIT, sz, 256 ); if( swscanf( sz, TEXT("%f"), &GetGlobalOptions().Absorption.g ) == 0 ) return false;
                    GetDlgItemText( hDlg, IDC_ABSORPTION_B_EDIT, sz, 256 ); if( swscanf( sz, TEXT("%f"), &GetGlobalOptions().Absorption.b ) == 0 ) return false;
...
void COptionsFile::ResetSettings()
{
    ...
    wcscpy_s( m_Options.strInputMesh, MAX_PATH, L"misc\\shapes1.x" );
    ...
    if( pLastSlash )
        *pLastSlash = 0;
    wcscpy_s( m_Options.strResultsFile, MAX_PATH, L"shapes1_prtresults.prt" );
    m_Options.dwOrder = 6;
    ...
    wcscpy_s( m_Options.strOutputMesh, MAX_PATH, L"shapes1_adaptive.x" );
...
void CPRTOptionsDlg::UpdateControlsWithSettings( HWND hDlg )
{
    ...
    CheckDlgButton( hDlg, IDC_SPECTRAL_CHECK, (GetGlobalOptions().dwNumChannels == 3) ? BST_CHECKED : BST_UNCHECKED );
    ...
    SetDlgItemText( hDlg, IDC_OUTPUT_EDIT, GetGlobalOptions().strResultsFile );
    SendMessage( GetDlgItem( hDlg, IDC_OUTPUT_EDIT ), EM_SETSEL, wcslen(GetGlobalOptions().strResultsFile), wcslen(GetGlobalOptions().strResultsFile) );
    SendMessage( GetDlgItem( hDlg, IDC_OUTPUT_EDIT ), EM_SCROLLCARET, 0, 0 );

    CheckRadioButton( hDlg, IDC_MESH_SAVE_BINARY_RADIO, IDC_MESH_SAVE_TEXT_RADIO, GetGlobalOptions().bBinaryOutputXFile ? IDC_MESH_SAVE_BINARY_RADIO : IDC_MESH_SAVE_TEXT_RADIO );
...
INT_PTR CALLBACK CPRTLoadDlg::DlgProc( HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam )
{
    switch( msg )
    {
        case WM_INITDIALOG:
        {
            ...
            SendMessage( GetDlgItem( hDlg, IDC_SIM_RESULTS_EDIT ), EM_SETSEL, wcslen(GetGlobalOptions().strResultsFile), wcslen(GetGlobalOptions().strResultsFile) );
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
                ...
                case IDOK:
                {
                    ...
                    HRESULT hr;
                    SetCurrentDirectory( GetGlobalOptions().strInitialDir );
                    if( FAILED( hr = DXUTFindDXSDKMediaFileCch( strResults, MAX_PATH, GetGlobalOptions().strResultsFile ) ) )
                    {
                        MessageBox( hDlg, TEXT("Couldn't find the simulator results file.  Run the simulator first to precompute the transfer vectors for the mesh."), TEXT("Error"), MB_OK );
                        break;
                    }

                    if( FAILED( hr = DXUTFindDXSDKMediaFileCch( strMesh, MAX_PATH, GetGlobalOptions().strInputMesh ) ) )
                    {
                        MessageBox( hDlg, TEXT("Couldn't find the mesh file.  Be sure the mesh file exists."), TEXT("Error"), MB_OK );
                        break;
                    }
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
                ...
                case IDOK:
                {
                    ...
                    GetGlobalOptions().dwRobustMeshRefineMaxSubdiv = (DWORD) LOWORD( SendMessage( GetDlgItem( hDlg, IDC_RMR_MAX_SUBD_SPIN ), UDM_GETPOS, 0, 0 ) );
                    GetGlobalOptions().dwAdaptiveDLMaxSubdiv = (DWORD) LOWORD( SendMessage( GetDlgItem( hDlg, IDC_DL_MAX_SUBD_SPIN ), UDM_GETPOS, 0, 0 ) );
                    GetGlobalOptions().dwAdaptiveBounceMaxSubdiv = (DWORD) LOWORD( SendMessage( GetDlgItem( hDlg, IDC_AB_MAX_SUBD_SPIN ), UDM_GETPOS, 0, 0 ) );
