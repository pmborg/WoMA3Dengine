...
//--------------------------------------------------------------------------------------
static HRESULT CreateTextureFromDDS( ID3D10Device* pDev, DDS_HEADER* pHeader, __inout_bcount(BitSize) BYTE* pBitData,
                                     UINT BitSize, __out ID3D10ShaderResourceView** ppSRV, bool bSRGB )
{
    HRESULT hr = S_OK;
        ...
        && (MAKEFOURCC( 'D', 'X', '1', '0' ) == pHeader->ddspf.dwFourCC ) )
    {
        ...
        if ( d3d10ext->arraySize > D3D10_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION )
            return HRESULT_FROM_WIN32( ERROR_NOT_SUPPORTED );
    ...
    D3D10_SUBRESOURCE_DATA* pInitData = new D3D10_SUBRESOURCE_DATA[iMipCount * desc.ArraySize];
    if( !pInitData )
        return E_OUTOFMEMORY;
...
//--------------------------------------------------------------------------------------
HRESULT CreateDDSTextureFromFile( ID3D10Device* pDev, const WCHAR* szFileName, ID3D10ShaderResourceView** ppSRV, bool bSRGB )
{
