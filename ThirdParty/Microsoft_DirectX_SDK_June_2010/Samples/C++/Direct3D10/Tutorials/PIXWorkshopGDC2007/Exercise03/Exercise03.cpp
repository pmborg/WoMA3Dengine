...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
{
    ...
    DXUTCreateWindow( L"PIX Workship GDC 2007: Exercise03" );
...
void VisibilityCullTiles()
{
    ...
    for( int i = TileCount - 1; i >= 0; i-- )
    {
        ...
        // Check the distance to the tile against the g_fFadeEnd distance.  If it is less, then
        // we can draw the grass.  Don't forget to take fTileRad into account, so that we don't
        // cull grass prematurely.
        //if( ??? < g_fFadeEnd )
        {
