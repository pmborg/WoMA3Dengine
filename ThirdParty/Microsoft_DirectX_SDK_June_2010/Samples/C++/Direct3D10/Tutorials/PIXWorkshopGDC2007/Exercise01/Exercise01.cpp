...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
{
    ...
    DXUTCreateWindow( L"PIX Workship GDC 2007: Exercise01" );
...
void InitApp()
{
    ...
    //-----------------------------------------------------------------------------------------
    // o/__   <-- BreakdancinBob TODO: This sets up our space partitioning for physics.  We
    // |  (\						   split up the world into X by Y grid cells.  Collisions
    //								   are then only performed on balls within a certain
    //								   proximity of each other.  Each ball in each grid cell
    //								   is tested against every other ball in that cell.
    //                          
    //                           HINT: For the default 2x2 grid we have on average 5000/4 (1250)
    //								   balls per grid cell.  For each grid cell, that means
    //								   we're doing 1250*1250 collision tests on average.  Even
    //								   with multiple threads working on it, this is still slow.
    //-----------------------------------------------------------------------------------------

    g_Sim.CreateCollisionGrid( 2, 2, g_fWorldScale );
