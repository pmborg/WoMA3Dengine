...
PSSceneIn VSInstancedSpectrogram(VSInstSceneIn input)
{
	...
	// half of that again.
	float fNormScale = lerp( 1, spectrogramslot, g_fNormalization );
	spectrogramslot /= 4.0;
	float4 spectrogram = g_txSpectrogram.SampleLevel( g_samLinear, float2(spectrogramslot,0), 0 );
	
//-----------------------------------------------------------------------------------------
// o/__   <-- BreakdancinBob TODO: Scale The height of the input vertex based on the  
// |  (\			   spectrogram level and offset the bar to the proper
//                                 position.
//                          
//                           HINT: The level you want to use is in the red channel of the
//                                 spectrogram variable.
//                           HINT: There are two sliders available in the UI for the app:
//					     Scale and Normalization. Scale is how big you want the
//                                 bars. Normalization is how much you want the visualizer 
//                                 to compensate for the fact that lower frequencies are 
//                                 almost always louder than higher frequencies. Both of 
//                                 these are available for you to use in computing the 
//                                 height of the bar - scale is stored in g_fScale, and 
//                                 the scaling factor needed to adjust the bar for 
//                                 normalization is calculated for you in fNormScale.
//-----------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------
// o/__   <-- BreakdancinBob TODO: Assign a different texture to each of the bars.  
// |  (\			       
//
//                           HINT: The bar being currently rendered is identified by 
//                                 input.InstanceId (i.e. the first bar's vertices all have
//                                 an input.InstanceId of 0, the second's have 1, etc.)            
//                           HINT: We've created 5 textures in an array that will be
//                                 sampled by the PS. When the PS goes to sample the array,
//                                 it will use the z component of output.Tex as the index 
//                                 into the array.
//-----------------------------------------------------------------------------------------
