...
// rendered frame.  The default effect creates a swirling effect.  However, playing with
// any of these parameters will give startlingly different results.  Experiment.
//-----------------------------------------------------------------------------------------
...
// Solution 1
...
#if 0
	// Create a reversing spirials effect by changing the lookup texture coordinates
	...
	// Blur it using a 3x3 box filter
	float4 color = BoxFilter( g_samLinearClamp, tex, 3 );
	
	// Return the result but faded out
	return color * 0.80;
...
#else
	// Try me for some very interesting XOR effects
	
	// Get the original input and a filtered input
	float2 tex = input.Tex.xy;
	float4 orig = g_txDiffuse.Sample( g_samLinearClamp, input.Tex.xy );
	float4 color = BoxFilter( g_samLinearClamp, tex, 5 );
	
	// move the colors up to int
	uint4 origInt = (uint4)(orig*80);
	uint4 colorInt = (uint4)(color*256);
	
	// XOR them
	uint4 xor = origInt ^ colorInt;
	
	// move them back down to float
	color = xor / 256.0;
