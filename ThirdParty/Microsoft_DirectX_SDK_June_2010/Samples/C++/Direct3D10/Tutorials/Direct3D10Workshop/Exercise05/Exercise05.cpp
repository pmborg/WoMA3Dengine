...
UINT                                g_NumGrowthSpurts = 7;
float                               g_fGrowth = 1.6f;
float                               g_LengthModifier = 0.70f;
float                               g_SpreadModifier = 0.1f;
float                               g_ShrinkModifier = 0.4f;
...
INT WINAPI WinMain( HINSTANCE, HINSTANCE, LPSTR, int )
{
    ...
    DXUTCreateWindow( L"D3D10 Shader Model 4.0 Workshop: Exercise05" );
                                      ...
                                      void* pUserContext )
{
    ...
    #endif
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, L"Exercise05.fx" ) );
    ...
    V_RETURN( pd3dDevice->CreateInputLayout( layout, 4, PassDesc.pIAInputSignature,
...
void CALLBACK OnD3D10FrameRender( ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    {
        //-----------------------------------------------------------------------------------------
        // o/__   <-- BreakdancinBob TODO:  Each call to GrowBranches invokes the GrowBranches
        // |  (\			 technique which adds one level of branches to the current tree. Calling
        //					 the function once grows one level of branches.  Modify the code below
        //					 to grow g_NumGrowthSpurts levels of branches.
        //-----------------------------------------------------------------------------------------	
    ...
    for( UINT p = 0; p < techDesc.Passes; p++ )
    {
        ...
        //-----------------------------------------------------------------------------------------
        // o/__   <-- BreakdancinBob TODO:  If we leave the draw call the way it is, we will only
        // |  (\			 draw g_NumIndices/3 triangles.  However, our streamed out geometry
        //					 will contain more than g_NumIndices/3 triangles.  Modify the draw call
        //					 below to draw the number of triangles contained in the g_pDrawFrom
        //					 buffer.
        //-----------------------------------------------------------------------------------------	
        pd3dDevice->DrawIndexed( g_NumIndices, 0, 0 );

        //-----------------------------------------------------------------------------------------
        // o/__   <-- BreakdancinBob HINT:  We have provided no mechanism to retrieve the number of
        // |  (\			 triangles created by GrowBranches.  It is possible to get this number,
        //					 but it requires reading data back from the GPU.  To avoid this use
