...
#include "../src-lib/GeoLite2PP.hpp"
#include "../src-lib/GeoLite2PP_error_category.hpp"
#include "../src-lib/GeoLite2PP_version.hpp"
...
MMDB_lookup_result_s GeoLite2PP::DB::lookup_raw( const std::string &ip_address )
{
	...
	if (gai_error)
	{
		...
		throw std::runtime_error("invalid_argument");//throw std::invalid_argument( gai_strerror(gai_error) );
	}
	if (mmdb_error)
	{
		/// @throw std::system_error if the lookup resulted in a MMDB error
