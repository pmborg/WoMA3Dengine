...
Tree			m_Trees[N_BILLBOARD + N_FENCES + N_FIRE];	// Array of tree info. NOTE: SHARED on 2 (BILLBOARD Instances)
...
xmlobj3d* BillClass::fillxml(int id, UINT type)
{
	...
	if (m_Trees[id].type == 100)
		strcpy_s(xmlobj.filename, 256, BILLBOARD_FENCE_MODEL);
	if (m_Trees[id].type == 200)
		strcpy_s(xmlobj.filename, 256, BILLBOARD_FIRE_MODEL);
