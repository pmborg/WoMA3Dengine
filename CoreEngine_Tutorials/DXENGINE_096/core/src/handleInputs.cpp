...
void DXInputClass::GetDirectInputKeys()
{
    ...
    if (m_ourPlayer->p_player.IsDownPressed)
        ...
        m_ourPlayer->p_player.IsUpPressed ||
        m_ourPlayer->p_player.IsLeftPressed ||
        m_ourPlayer->p_player.IsRightPressed
        )
    {
        if (m_ourPlayer->p_player.IsLeftCtrlPressed)
            USING_CHAR_MODEL_ANIMATION = 3; //run
        else
            USING_CHAR_MODEL_ANIMATION = 1; //walk
    }
    else
        USING_CHAR_MODEL_ANIMATION = 0;
	...
	//COMPOUND DEBUG:
#if DX_ENGINE_LEVEL >= 89 && defined USE_MAP_EDITOR && defined _DEBUG // #if TUTORIAL_PRE_CHAP >= 12
    m_ourPlayer->p_player.IsNumPadMinus     = (m_keyboardState[DIK_SUBTRACT] & 0x80) ? true : false;									
    m_ourPlayer->p_player.IsNumPadPlus      = (m_keyboardState[DIK_ADD] & 0x80) ? true : false;											//COMPOUND DEBUG:
    m_ourPlayer->p_player.IsShift           = (m_keyboardState[DIK_LSHIFT] & 0x80) || (m_keyboardState[DIK_RSHIFT] & 0x80) ? true : false;	//COMPOUND DEBUG:
    m_ourPlayer->p_player.IsRctrlPressed    = (m_keyboardState[DIK_RCONTROL] & 0x80) ? true : false;									//COMPOUND DEBUG:
#endif
