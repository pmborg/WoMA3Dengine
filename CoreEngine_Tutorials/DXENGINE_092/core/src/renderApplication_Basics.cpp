...
void ApplicationClass::SortOutWhatNeedToBeRendered(void* pContext, WomaDriverClass* driver)
{
	...
	WOMA::sceneManager->visibleBillboardList.clear();
	...
	std::sort(WOMA::sceneManager->visibleBillboardList.begin(), WOMA::sceneManager->visibleBillboardList.end(), BillSortCB_CPP);
#endif
...
void ApplicationClass::RenderModel(void* pContext, UINT threadID, UINT monitorIndex, WomaDriverClass* driver, UINT ID, UINT pass, XMMATRIX* m_viewMatrix, XMMATRIX* m_projectionMatrix)
{
	...
	if (threadID == 0)
	{
		if (pass == PASS_OPAC)
		{
			...
			if (WOMA::sceneManager->visibleBillboardList.size() == 0)
				return;
			modelID = ID;
			pass = PASS_OPAC;
		}
   ...
   ((DXmodelClass*)model)->visible = true;
	...
	if (m_viewMatrix == NULL && m_projectionMatrix == NULL)
		model->Render(pContext, threadID, CAMERA_NORMAL, PROJECTION_PERSPECTIVE, pass, &(m_Light->m_viewMatrix), &(m_Light->m_ligth_orthoMatrix));
    else
...
// --------------------------------------------------------------------------------------------
    ...
    ((DirectX::DXmodelClass*)m_TerrainModel[WATER_TERRAIN_ID])->m_Shader11->time += (float)SystemHandle->m_Application->dt * (0.0025f / 16.66f);
    m_TerrainModel[WATER_TERRAIN_ID]->translation(0, -0.75, 0);
    m_TerrainModel[WATER_TERRAIN_ID]->scale(5, 5, 5);
	m_TerrainModel[WATER_TERRAIN_ID]->RenderWithFade(pContext, fadeLight, false);
#endif
