...
{
	CLASS_LOAD_N--; 
	CLASSLOADER();  //don't count super class!
	WomaIntegrityCheck = 1234525217;
						...
						BOOL msaa, bool vsync, BOOL fullscreen, BOOL g_UseDoubleBuffering, BOOL g_AllowResize)
{
    ...
    pixelFormatDescriptor.cColorBits = depthBits;	//Default: 24 //use to be: 16
    pixelFormatDescriptor.cDepthBits = depthBits;	//Default: 24 //use to be: 15
...
*/
	...
	// Turn on or off the vertical sync depending on the input
	// -------------------------------------------------------
	...
	bool HasPixelFormatARB = false;
	bool HasMultisample = false;
	bool HasHardwareGamma = false;
