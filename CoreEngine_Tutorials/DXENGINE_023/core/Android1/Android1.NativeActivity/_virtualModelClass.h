...
public:
	...
	virtual void RenderWithFade(WomaDriverClass* driver, float fadeLight = 1, bool FOG = false) = 0;
#if defined USE_SKY_DOME && DX_ENGINE_LEVEL >= 28 && defined USE_SKYSPHERE
...
#if DX_ENGINE_LEVEL >= 21 ||  defined INTRO_DEMO
...
#endif
#if DX_ENGINE_LEVEL >= 53 // ENGINE_LEVEL >= 66
	virtual bool LoadTerrain(TCHAR* objectName, void* driver, SHADER_TYPE shader_type, std::vector<STRING> *textureFile, std::vector<ModelTextureDouble_Color_TerrainType> *model, std::vector<UINT>* indexList = NULL, UINT instanceCount = 0) = 0;
#endif
	...
	INT					m_ObjId=-1;
	SHADER_TYPE			ModelShaderType= SHADER_AUTO;
