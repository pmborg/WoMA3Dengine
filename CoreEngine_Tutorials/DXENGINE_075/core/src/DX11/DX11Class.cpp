...
//----------------------------------------------------------------------------------------------
{
#if defined USE_DX10DRIVER_FONTS // || defined INTRO_DEMO	// FONT v2
...
//----------------------------------------------------------------------------------------------
{
	...
	//Release the two new blending states.
		...
		// The Last one!
		#if _DEBUG
		DXGI_DEBUG_RLO_FLAGS flags = DXGI_DEBUG_RLO_DETAIL;//DXGI_DEBUG_RLO_ALL;
		if (debugDev)
		HRESULT hr = debugDev->ReportLiveObjects(DXGI_DEBUG_ALL, flags);
		#endif
...
// ----------------------------------------------------------------------------------------------
{
	...
	if (DX11windowsArray[monitorWindow].m_swapChain)
		DX11windowsArray[monitorWindow].m_swapChain->Present(m_VSYNC_ENABLED, 0);
	else
		DX11windowsArray[monitorWindow].m_swapChain1->Present(m_VSYNC_ENABLED, 0);
...
// ----------------------------------------------------------------------------------------------
{
	...
	*******************************************************************/
	...
	// MINI-MAP:
	m_projectionMiniMapMatrix = XMMatrixPerspectiveFovLH( fieldOfView, 1, screenNear, 1024);    //Normal 3D PROJECTION
}
...
// ----------------------------------------------------------------------------------------------
{
	...
	DXsystemHandle->m_CameraSKY->SetRotation(SystemHandle->AppSettings->INIT_ROTX,
...
XMMATRIX* DX11Class::GetViewMatrix( UINT camera, UINT projection, UINT pass, void* lightViewMatrix, void* ShadowProjectionMatrix)
{
	...
	if (pass == PASS_SHADOWS)
		return (XMMATRIX*)lightViewMatrix;
	else
	#endif
	{
		switch (projection)
...
XMMATRIX* DX11Class::GetProjectionMatrix( UINT camera, UINT projection, UINT pass, void* lightViewMatrix, void* ShadowProjectionMatrix)
{
	#if DX_ENGINE_LEVEL >= 36 && defined USE_SHADOW_MAP
	if (pass == PASS_SHADOWS)
		return (XMMATRIX*)ShadowProjectionMatrix;
	else
	#endif
	{
		switch (projection)
