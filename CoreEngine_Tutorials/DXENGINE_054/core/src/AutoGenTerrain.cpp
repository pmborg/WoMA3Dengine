...
// ----------------------------------------------------------------------------
{
		...
		if (terrainId == 2)
	{
		...
		for (UINT i = 0; i < modelVertexVector2.size(); i++)				// Num Vertices: 6x256x256 //{ size=6303750 }
		{
			...
			modelVertexVector2[i].y = modelVertexVector2[i].y / 20 - 0.1f;	// Scale Altitude
			if (modelVertexVector2[i].y <= 0)
				modelVertexVector2[i].y = -2;
		}
	}
	...
	if (terrainId == 3) {
			...
			for (UINT i = 0; i < modelVertexVector3.size(); i++)			// Num Vertices: 6x256x256
			{
				...
				modelVertexVector3[i].y = modelVertexVector3[i].y / 20 - 0.1f;
				if (modelVertexVector3[i].y <= 0)
					modelVertexVector3[i].y = -2;
			}
		}
		#endif
	#endif
	...
	for (UINT y = 0; y < terrain_squares ; y++) {
		...
		for (UINT x = 0; x < terrain_squares; x++)
		{
			...
			if (id == 2) {
				...
				if (x == 0 || x == terrain_squares - 1 || y == 0 || y == terrain_squares - 1)	// SET: Fixed border / Terrain edgeds
					vertex2.y = -1;
				else
...
// ----------------------------------------------------------------------------
{
...
#if defined SCENE_MAIN_TOPO_TERRAIN_USE_INDEX
	if (id == 0) ASSERT(SystemHandle->m_Application->m_TerrainModel[id]->LoadTexture(pContext, TEXT("id0:under water"), m_Driver, shader_type, &Textures, &modelVertexVector0, NULL, 0));
	if (id == 1) ASSERT(SystemHandle->m_Application->m_TerrainModel[id]->LoadTexture(pContext, TEXT("id1:water"), m_Driver, shader_type, &Textures, &modelVertexVector1, NULL, 0));
	if (id == 2) ASSERT(SystemHandle->m_Application->m_TerrainModel[id]->LoadTerrain(pContext, TEXT("id2:terrain"), m_Driver, shader_type, &Textures, &modelVertexVector2, &indices, 0));
#if defined DEBUG_COLLISION_TERRAIN
	//ModelTextureVertexType
	//float x, y, z;
	//float tu, tv;
	if (id == 3) ASSERT(SystemHandle->m_Application->m_TerrainModel[id]->LoadTexture(pContext, TEXT("id3:terrain"), m_Driver, shader_type, &Textures, &modelVertexVector3, &indices));
#endif
#else
	...
	if (id == 1) ASSERT(SystemHandle->m_Application->m_TerrainModel[id]->LoadTexture(pContext, TEXT("id1:water"), m_Driver, shader_type, &Textures, &modelVertexVector1));
	if (id == 2) ASSERT(SystemHandle->m_Application->m_TerrainModel[id]->LoadTerrain(pContext, TEXT("id2:terrain"), m_Driver, shader_type, &Textures, &modelVertexVector2));
#endif
