...
typedef struct tagBITMAPFILEHEADER {
	uint16_t    bfType{ 0x4D42 };	// 2          // File type always BM which is 0x4D42
	uint32_t 	bfSize{ 0 };		// 4          // Size of the file (in bytes)
	uint16_t    reserved1{ 0 };		// 2	      // Reserved, always 0
	uint16_t    reserved2{ 0 };		// 2	      // Reserved, always 0
	uint32_t 	bfOffBits{ 0 };		// 4	      // Start position of pixel data (bytes from the beginning of the file)
...
struct TargaHeader
{
	...
	*/
	unsigned short	width = 0;	//Image Specification
	unsigned short	height = 0;	//Image Specification
	unsigned char	bpp = 0;		//Image Specification
	unsigned char	attrib = 0;	//Image Specification
};
...
public:
	...
	unsigned char* loadBMP	(TCHAR* filename, UINT* width, UINT* height, UINT* imageSize, UINT loadimagebits = LOAD_IMAGE_32bits, bool invertImage = true);
#endif
