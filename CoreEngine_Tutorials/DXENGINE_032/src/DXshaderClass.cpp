	...
	// Create the Vertex Shader from the buffer: (GET CODE ON "EXE")
		...
		if (SystemHandle->AppSettings->DRIVER == DRIVER_DX12)
		{
			...
			case SHADER_TEXTURE_LIGHT_RENDERSHADOW:
			{
				...
				srvHeapDesc.NumDescriptors = 2 + 10; //NOTE: Allow max: 10 Texture per object in DX12... if need more need to increase here!
				break;
			}
			}
			...
			*/
			{
				...
				BOOL alphaBlend = driver->g_AlphaBlend;
			...
			case SHADER_TEXTURE_LIGHT_RENDERSHADOW:
			{
				...
				// 2 CBV
				offSet.ptr += m_CbvSrvDescriptorSize;
				driver->m_commandList->SetGraphicsRootDescriptorTable(1, offSet);	//C1 Set "Constant Buffer View" 

				// SRV:
				offSet.ptr += m_CbvSrvDescriptorSize;
				for (size_t i = 0; i < texture_index; i++)
					offSet.ptr += m_CbvSrvDescriptorSize;
				driver->m_commandList->SetGraphicsRootDescriptorTable(2, offSet);	//T0 Set Texture
				break;
			}
			}
