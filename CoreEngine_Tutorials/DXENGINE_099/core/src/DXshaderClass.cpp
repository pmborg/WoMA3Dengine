...
#if defined GENERATE_ATLAS_INTEGRATION_DDS
#if defined DX11 || defined DX9
static const D3D11_INPUT_ELEMENT_DESC billboardAtlasLayout11[] =
{
	{ "POSITION",  0, DXGI_FORMAT_R32G32B32_FLOAT,	0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },								// Position: 3 floats
	{ "TEXCOORD",  0, DXGI_FORMAT_R32G32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },	// TexCoord: 2 floats (original quad UVs)
	{ "NORMAL",    0, DXGI_FORMAT_R32G32B32_FLOAT,	0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },	// Normal: 3 floats (optional but useful for lighting / facing)
	{ "TEXCOORD",  1, DXGI_FORMAT_R32_UINT,			0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },	// Atlas Index (TEXCOORD1)
	{ "TEXCOORD",  2, DXGI_FORMAT_R32G32B32_FLOAT,	0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },	// origin
	{ "TEXCOORD",  3, DXGI_FORMAT_R32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },	// scale
	{ "TEXCOORD",  4, DXGI_FORMAT_R32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },	// rotY

};
#endif

#if defined DX12 && D3D11_SPEC_DATE_YEAR > 2009
static const D3D12_INPUT_ELEMENT_DESC billboardAtlasLayout[] =
{
	{ "POSITION",  0, DXGI_FORMAT_R32G32B32_FLOAT,	0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },	// Position: 3 floats
	{ "TEXCOORD",  0, DXGI_FORMAT_R32G32_FLOAT,		0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },		// TexCoord: 2 floats
	{ "NORMAL",    0, DXGI_FORMAT_R32G32B32_FLOAT,	0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },	// Normal: 3 floats
	{ "TEXCOORD",  1, DXGI_FORMAT_R32_UINT,			0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },	// Atlas Index (TEXCOORD1)
	{ "TEXCOORD",  2, DXGI_FORMAT_R32G32B32_FLOAT,	0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },	// origin
	{ "TEXCOORD",  3, DXGI_FORMAT_R32_FLOAT,		0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },	// scale
	{ "TEXCOORD",  4, DXGI_FORMAT_R32_FLOAT,		0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },	// rotY
};
#endif
#endif
...
static const D3D11_INPUT_ELEMENT_DESC mappingDetailligthcolorPolygonLayout11[] =
{
	{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,	0, 0,							 D3D11_INPUT_PER_VERTEX_DATA, 0 },
...
static const D3D11_INPUT_ELEMENT_DESC waterfallLayout11[] =
{
	{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,	0, 0,							 D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,		0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },//16 Bits (RG)
	{ "COLOR",    0, DXGI_FORMAT_R32G32B32A32_FLOAT,0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
...
static const D3D11_INPUT_ELEMENT_DESC mappingDetailligthcolorBumpPolygonLayout11[] =
{
	{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT,	0, 0,							 D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "TEXCOORD", 0, DXGI_FORMAT_R32G32B32A32_FLOAT,0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	{ "NORMAL",	  0, DXGI_FORMAT_R32G32B32_FLOAT,   0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 },
...
#define device11 ((ID3D11Device*)DriverDevice)
	...
	// Initialize the vertex and pixel shaders.
		...
		case SHADER_NORMAL_BUMP:
			...
			break;
        case SHADER_TEXTURE_GS_INSTANCED:
            polygonLayout11 = &lightGSNormalPolygonLayout11[0];
            numElements = sizeof(lightGSNormalPolygonLayout11) / sizeof(lightGSNormalPolygonLayout11[0]);
            break;
		...
		case SHADER_TEXTURE_LIGHT_SAVESHADOW_INSTANCED:
			...
			break;
#endif
		case SHADER_Double_Color_Terrain:						//053Double_Color_Terrain.hlsl
		case SHADER_Slope_Texture_Terrain:						//055Slope_Texture_Terrain.hlsl
			polygonLayout11 = &ligthcolorPolygonLayout11[0];
			numElements = sizeof(ligthcolorPolygonLayout11) / sizeof(ligthcolorPolygonLayout11[0]);
			break;
		case SHADER_Terrain_Texture_DEMO19:			//056Terrain.hlsl
			polygonLayout11 = &mappingDetailligthcolorPolygonLayout11[0];
			numElements = sizeof(mappingDetailligthcolorPolygonLayout11) / sizeof(mappingDetailligthcolorPolygonLayout11[0]);
			break;
...
#endif
#if defined GENERATE_ATLAS_INTEGRATION_DDS
		case SHADER_BILLBOARD_ATLAS_FAST:       // 093BillboardAtlas.hlsl
			polygonLayout11 = &billboardAtlasLayout11[0];
			numElements = sizeof(billboardAtlasLayout11) / sizeof(billboardAtlasLayout11[0]);
			break;
#endif
...
//NEW SHADER:
		...
		case SHADER_Terrain_Texture_DEMO99:
			vsFilename.append(L"hlsl/099Terrain.hlsl");
			psFilename = vsFilename;
			vertexHLSL.append("VS_Main");
			pixelHLSL.append("PS_Main");
			break;
		...
		case SHADER_FIRE:
			...
			break;
        case SHADER_TEXTURE_GS_INSTANCED:
            vsFilename.append(L"hlsl/077GSinstance.hlsl");
            psFilename = vsFilename;
            vertexHLSL.append("VS_Main");
#if DX_ENGINE_LEVEL >= 90 && defined USE_OPTIMIZING
            gsHLSL.append("GS_Main");
            //bUseGS = true;
#endif
            pixelHLSL.append("PS_Main");
        ...
        case SHADER_TEXTURE_LIGHT_FAST:
            vsFilename.append(L"hlsl/083Light.hlsl");
            psFilename = vsFilename;
            vertexHLSL.append("VS_Main");
            pixelHLSL.append("PS_Main");
            break;
		...
		case SHADER_BILLBOARD_ATLAS_FAST:
			vsFilename.append(L"hlsl/093BillboardAtlas.hlsl");
			psFilename = vsFilename;
			vertexHLSL.append("VS_Main");
			pixelHLSL.append("PS_Main");
			break;
		...
		case SHADER_USE_SMOKE:
			vsFilename.append(L"hlsl/097smoke.hlsl");
			psFilename = vsFilename;
			vertexHLSL.append("VS_Main");
			pixelHLSL.append("PS_Main");
			break;
...
//93
		...
		if (WOMA::AppSettings->DRIVER == DRIVER_DX11 || WOMA::AppSettings->DRIVER == DRIVER_DX9)
		{
...
#if DX_ENGINE_LEVEL >= 90 && defined USE_OPTIMIZING
            if (bUseGS) {
            vertVer[0] = 'g';  //TEXT("gs_5_0")
            result = D3DCompileFromFile(file, defines, D3D_COMPILE_STANDARD_FILE_INCLUDE, gsHLSL.c_str(), /*"ps_5_0"*/vertVer.c_str(), compileFlags, 0, &geometryShaderBuffer, &errorMessage);
            if (FAILED(result))
            {
                WomaMessageBox((TCHAR*)errorMessage->GetBufferPointer(), TEXT("SHADER Error description :"));
                if (errorMessage)
            ...
            if (bUseGS)
            {
                ...
                result = device11->CreateGeometryShader(geometryShaderBuffer->GetBufferPointer(), geometryShaderBuffer->GetBufferSize(), NULL, &m_geometryShader11);
                if (FAILED(result))
                    { WomaMessageBox(TEXT("ERROR"), TEXT("CreateGeometryShader:")); return false; }
            }
#endif
...
// END 99 TYPEs
		...
		if (m_shaderType == SHADER_BILLBOARD_ATLAS_FAST)
		{
			...
			HRESULT result = deviceContext11->Map(m_VertexShaderBuffer11, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
			if (FAILED(result)) { throw woma_exception("Map failed", __FILE__, __FUNCTION__, __LINE__); }

			dataVSptr = (VSBillboardAtlasConstantBufferType*)mappedResource.pData;

			// MATRICES
			dataVSptr->world = XMMatrixTranspose(*worldMatrix);
			XMMATRIX WV = (*worldMatrix) * (*viewMatrix);
			dataVSptr->WVP = XMMatrixTranspose(WV * (*projectionMatrix));

			// LIGHTING (already available in LightClass)
			dataVSptr->lightDirection = SystemHandle->m_Application->app_Light->m_lightDirection;

			// VEGETATION WIND ANIMATION
			dataVSptr->vsframeTime = SystemHandle->m_Application->shadergrassframeTime;
			dataVSptr->scrollSpeeds = scrollSpeeds;   // existing variable
			dataVSptr->scales = scales;         // existing variable
			dataVSptr->isAnimatedBill = isAnimatedBill;
			dataVSptr->fade = PSfade;

			// Make sure we don't copy more than we have
			size_t regionCount = min(billboardAtlasRegions.size(), (size_t)MAX_ATLAS_REGIONS);
			memcpy(
				dataVSptr->billboardAtlasRegions,
				billboardAtlasRegions.data(),  // <-- FIX: Access raw pointer
				sizeof(AtlasRegion) * regionCount
			);

			// Unmap and bind buffer
			deviceContext11->Unmap(m_VertexShaderBuffer11, 0);
			deviceContext11->VSSetConstantBuffers(0, 1, &m_VertexShaderBuffer11);
			deviceContext11->PSSetConstantBuffers(0, 1, &m_VertexShaderBuffer11);
...
// END: SHADER_BILLBOARD_ATLAS_FAST
		...
		if (m_shaderType == SHADER_USE_WATERFALL)
		{
			...
			dataVSptr = (VSWaterfallConstantBufferType*)mappedResource.pData;

			// MATRICES
			XMMATRIX WV = (*worldMatrix) * (*viewMatrix);
			dataVSptr->World = *worldMatrix;
			dataVSptr->WVP = XMMatrixTranspose(WV * (*projectionMatrix));
			dataVSptr->time = SystemHandle->m_Application->dt*0.0001f;
			dataVSptr->scrollSpeed = 0.1f;		// Range: 0.1 – 1.0.
			dataVSptr->noiseStrength = 0.1f;	// Range: 0.0 – 0.2.
			dataVSptr->foamY = 15;				// translation(138, 15, 57)
			dataVSptr->foamColor = XMFLOAT4(120.0f / 256.0f, 220.0f / 256.0f, 255.0f / 255.0f, 0.8f);
			dataVSptr->fade = PSfade;

			// Un-map and bind buffer
			deviceContext11->Unmap(m_VertexShaderBuffer11, 0);
			deviceContext11->VSSetConstantBuffers(0, 1, &m_VertexShaderBuffer11);
			deviceContext11->PSSetConstantBuffers(0, 1, &m_VertexShaderBuffer11);
		...
		if (m_shaderType == SHADER_USE_SMOKE)
		{
			// --- 1) Update the VS constant-buffer (WVP) ---
			D3D11_MAPPED_SUBRESOURCE mappedResource = {};
			HRESULT hr = deviceContext11->Map(m_VertexShaderBuffer11, 0,
				D3D11_MAP_WRITE_DISCARD, 0,
				&mappedResource);
			if (FAILED(hr))
				throw woma_exception("Map failed", __FILE__, __FUNCTION__, __LINE__);

			// Shader’s b0 structure
			struct VSSmokeCB
			{
				XMMATRIX WVP;
				XMFLOAT3 cameraPos;  // present in shader but unused, fill with 0
				float	 fade;
			};

			auto* cb = reinterpret_cast<VSSmokeCB*>(mappedResource.pData);
			cb->WVP = XMMatrixTranspose((*worldMatrix) * (*viewMatrix) * (*projectionMatrix));
			...
			if (!m_particleAlphaCB)
			{
				...
				HRESULT hr = device11->CreateBuffer(&cbd, nullptr, &m_particleAlphaCB);
				if (FAILED(hr))
					WomaFatalExceptionW(TEXT("Failed to create SmokeParticleCB"));

			}
			{
				deviceContext11->UpdateSubresource(m_particleAlphaCB, 0, nullptr, &m_particleAlpha, 0, 0);
				deviceContext11->PSSetConstantBuffers(1, 1, &m_particleAlphaCB);
			}
...
//END: SHADER_USE_WATERFALL
	...
	// ----------------------------------------------------------------------------
		...
		if (hasFog)
		{
			dataVSptr->VSfogStart = fogStart;
			dataVSptr->VSfogEnd = fogEnd;
		}
...
#define deviceContext ((ID3D11DeviceContext*)Device_Context)
			...
			if (m_shaderType == SHADER_Terrain_Texture_DEMO99)
				deviceContext->PSSetSamplers(2, 1, &m_sampleStateClamp11); // 2, 1 or 0, 2
	...
	void DXshaderClass::RenderSmoke(UINT pass, void* Device_Context, int indexCount, XMMATRIX* worldMatrix, XMMATRIX* viewMatrix, XMMATRIX* projectionMatrix, float m_particleAlpha)
	{
		...
		SetShaderParameters(pass, Device_Context, worldMatrix, viewMatrix, projectionMatrix, NULL, NULL, m_particleAlpha);	// Set the shader parameters that it will use for rendering
		RenderShader(pass, Device_Context, /*texture_index*/ 0, indexCount);					// Now render the prepared buffers with the shader
	}

#if defined USE_FASTER_BILL_SHADER
	// ----------------------------------------------------------------------------------------
	void DXshaderClass::SetFASTShaderParameters(UINT pass, void* Device_Context,
		XMMATRIX* worldMatrix, XMMATRIX* viewMatrix, XMMATRIX* projectionMatrix)
	// ----------------------------------------------------------------------------------------
	{
		HRESULT result;
		...
		if (WOMA::AppSettings->DRIVER == DRIVER_DX11 || WOMA::AppSettings->DRIVER == DRIVER_DX9)
		{
			...
			dataVSptr = (VSFASTconstantBufferType*)mappedResource.pData;	// Get a pointer to the data in the constant buffer.
		}
#endif
		...
		// Copy the matrices into the constant buffer.
		dataVSptr->world = XMMatrixTranspose(*worldMatrix);
		dataVSptr->WVP = XMMatrixTranspose((*worldMatrix) * (*viewMatrix) * (*projectionMatrix));	// Pre compute WVP to reuse in all Vertices
