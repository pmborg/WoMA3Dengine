...
#if defined GENERATE_ATLAS_INTEGRATION_DDS
	billboardAtlasRegionsSRV = nullptr;
#endif
}

void DXmodelClass::SetAlpha(float a)
{
	g_particleAlpha = a;
}
...
#if defined GENERATE_ATLAS_INTEGRATION_DDS
bool DirectX::DXmodelClass::LoadBillboardAtlas(void* pContext, TCHAR* objectName, void* driver,
	SHADER_TYPE shader_type, std::vector<STRING>* textureFile,
	std::vector<ModelBillboardAtlasVertexType>* model, std::vector<UINT>* indexList, UINT instanceCount)
{
	LOADDRIVER(driver);
	MODEL_NAME = objectName;
	...
	modelBillboardAtlasVertex = model;

	ASSERT(ModelShaderType == SHADER_BILLBOARD_ATLAS_FAST);
...
bool DirectX::DXmodelClass::LoadWaterFall(void* pContext,
	TCHAR* objectName, void* driver,
	SHADER_TYPE shader_type,
	std::vector<STRING>* textureFile, std::vector<ModelTextureWaterfall>* model,
	std::vector<UINT>* indexList, UINT instanceCount)
{
	LOADDRIVER(driver);
	MODEL_NAME = objectName;
	...
	if (shader_type == SHADER_AUTO)
		ModelShaderType = SHADER_USE_WATERFALL;
	else
		ModelShaderType = shader_type;

	ASSERT(ModelShaderType == SHADER_USE_WATERFALL);

	modelTextureWaterfall = model;
	indexModelList = indexList;
	return InitializeDXbuffers((ID3D11DeviceContext*)pContext, objectName, textureFile);
}
bool DirectX::DXmodelClass::LoadSmoke(void* pContext,
	TCHAR* objectName, void* driver,
	SHADER_TYPE shader_type,
	std::vector<STRING>* textureFile, std::vector<SmokeVertexType>* model,
	std::vector<UINT>* indexList, UINT instanceCount)
{
	LOADDRIVER(driver);
	MODEL_NAME = objectName;
	...
	if (shader_type == SHADER_AUTO)
		ModelShaderType = SHADER_USE_SMOKE;
	else
		ModelShaderType = shader_type;

	ASSERT(ModelShaderType == SHADER_USE_SMOKE);
...
DXshaderClass* DXmodelClass::CreateShader(TCHAR* objectName, SHADER_TYPE ShaderType)
{
	...
	case SHADER_BILLBOARD_ATLAS_FAST:
		...
		ASSERT_DEBUG(m_vertexCount > 0);										// Better check, if object is empty...
		break;
#endif
	...
	case SHADER_USE_SMOKE:
		m_vertexCount = (UINT)(*modelSmoke).size();						// Set the number of vertices in the vertex array.
		ASSERT_DEBUG(m_vertexCount > 0);								// Better check, if object is empty...
		break;
#endif
...
bool DirectX::DXmodelClass::InitializeDXbuffers(ID3D11DeviceContext* pContext, TCHAR* objectName, std::vector<STRING>* textureFile)
{
	...
	case SHADER_BILLBOARD_ATLAS_FAST:
	#if defined DX11
		result = InitializeBillboardAtlasBuffers(m_driver11->m_device11, indices);
		break;
	#elif defined DX12
		result = InitializeBillboardAtlasBuffers(m_driver->m_device, indices);
		break;
	#endif
	#endif
	...
	case SHADER_USE_WATERFALL:
		result = InitializeTextureWaterfall(m_driver11->m_device11, indices);
		break;
	case SHADER_USE_SMOKE:
		result = InitializeTextureSmoke(m_driver11->m_device11, indices);
		break;
	...
	#if defined GENERATE_ATLAS_INTEGRATION_DDS
		if (ModelShaderType == SHADER_BILLBOARD_ATLAS_FAST)
	
		{
			meshSRV11.push_back(billboardAtlasSRV);
			return true;  // Skip loading individual textures
		} else
	#endif
		...
		if (meshSRV_size == 0 && textureFile)
		{
			for (UINT i = 0; i < (*textureFile).size(); i++)
			{
				...
				if (fileNamePath.find(TEXT("none")) != 0) //don't load on special cases (like billboards)
				{
					...
					HRESULT res = LoadTextureImage(pContext, (TCHAR*)fileNamePath.c_str());
...
#if defined GENERATE_ATLAS_INTEGRATION_DDS
bool DXmodelClass::InitializeBillboardAtlasBuffers(ID3D11Device* device, const std::vector<AtlasRegion>& regions)
{
	HRESULT hr;

	// Release previous SRV if it exists (safe reset)
	SAFE_RELEASE(billboardAtlasRegionsSRV);

	// Describe the structured buffer
	D3D11_BUFFER_DESC bufferDesc = {};
	bufferDesc.Usage = D3D11_USAGE_DEFAULT;
	bufferDesc.ByteWidth = static_cast<UINT>(regions.size() * sizeof(AtlasRegion));
	bufferDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	bufferDesc.CPUAccessFlags = 0;
	bufferDesc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
	bufferDesc.StructureByteStride = sizeof(AtlasRegion);

	// Fill the initial buffer with the atlas regions
	D3D11_SUBRESOURCE_DATA initData = {};
	initData.pSysMem = regions.data();

	ID3D11Buffer* structuredBuffer = nullptr;
	hr = device->CreateBuffer(&bufferDesc, &initData, &structuredBuffer);
	if (FAILED(hr))
	{
		womalog(TEXT("Failed to create BillboardAtlas structured buffer"));
		return false;
	}

	// Describe the SRV for the structured buffer
	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_BUFFER;
	srvDesc.Format = DXGI_FORMAT_UNKNOWN; // Structured buffers must use UNKNOWN
	srvDesc.Buffer.FirstElement = 0;
	srvDesc.Buffer.NumElements = static_cast<UINT>(regions.size());

	hr = device->CreateShaderResourceView(structuredBuffer, &srvDesc, &billboardAtlasRegionsSRV);
	SAFE_RELEASE(structuredBuffer); // We don’t need the buffer handle anymore

	if (FAILED(hr))
	{
		womalog(TEXT("Failed to create BillboardAtlasRegions SRV"));
		return false;
	}
...
#if defined GENERATE_ATLAS_INTEGRATION_DDS
bool DXmodelClass::InitializeBillboardAtlasBuffers(void* device, void* indices)
{
	// Get vertex count from modelBillboardAtlasVertex vector
	m_vertexCount = (UINT)(*modelBillboardAtlasVertex).size();

	// Allocate DX‑friendly vertex array
	DXbillboardAtlasVertexType* vertices = NEW DXbillboardAtlasVertexType[m_vertexCount];
	IF_NOT_THROW_EXCEPTION(vertices);

	// Fill DX/OpenGL vertex buffer from WoMA vertices
	for (UINT i = 0; i < m_vertexCount; i++)
	{
		vertices[i].position = XMFLOAT3((*modelBillboardAtlasVertex)[i].x,
			(*modelBillboardAtlasVertex)[i].y,
			(*modelBillboardAtlasVertex)[i].z);

		vertices[i].texCoord = XMFLOAT2((*modelBillboardAtlasVertex)[i].tu,
			(*modelBillboardAtlasVertex)[i].tv);

		vertices[i].normal = XMFLOAT3((*modelBillboardAtlasVertex)[i].nx,
			(*modelBillboardAtlasVertex)[i].ny,
			(*modelBillboardAtlasVertex)[i].nz);

		vertices[i].atlasIndex = (*modelBillboardAtlasVertex)[i].atlasIndex;

		vertices[i].origin = XMFLOAT3(	(*modelBillboardAtlasVertex)[i].ox,
										(*modelBillboardAtlasVertex)[i].oy,
										(*modelBillboardAtlasVertex)[i].oz);

		vertices[i].scale = (*modelBillboardAtlasVertex)[i].scale;
		vertices[i].rotY = (*modelBillboardAtlasVertex)[i].rotY;
	...
	// Create DX/OpenGL GPU buffers
	IF_NOT_RETURN_FALSE(CreateDXbuffers(sizeof(DXbillboardAtlasVertexType), device, indices, vertices));
...
bool DXmodelClass::InitializeTextureNormalBumpBuffers(/*ID3D11Device*/ void* device, UINT* indices)
{
	...
	for (UINT i = 0; i < m_vertexCount; i++)
	{
		...
		womalog("BUMP vertex[i]: %d\n", i);
		womalog("BUMP vertex[i].X: %5.1f\n", vertices[i].position.x);
		womalog("BUMP vertex[i].Y: %5.1f\n", vertices[i].position.x);
		womalog("BUMP vertex[i].Z: %5.1f\n", vertices[i].position.x);

		womalog("BUMP vertex[i].U: %5.1f\n", vertices[i].texCoord.x);
		womalog("BUMP vertex[i].V: %5.1f\n", vertices[i].texCoord.y);

		womalog("BUMP vertex[i].nX: %5.1f\n", vertices[i].normal.x);
		womalog("BUMP vertex[i].nY: %5.1f\n", vertices[i].normal.x);
		womalog("BUMP vertex[i].nZ: %5.1f\n", vertices[i].normal.x);

		womalog("BUMP vertex[i].tX: %5.1f\n", vertices[i].tangent.x);
		womalog("BUMP vertex[i].tY: %5.1f\n", vertices[i].tangent.x);
		womalog("BUMP vertex[i].tZ: %5.1f\n", vertices[i].tangent.x);

		womalog("BUMP vertex[i].bX: %5.1f\n", vertices[i].binormal.x);
		womalog("BUMP vertex[i].bY: %5.1f\n", vertices[i].binormal.x);
		womalog("BUMP vertex[i].bZ: %5.1f\n", vertices[i].binormal.x);
...
bool DXmodelClass::InitializeTextureWaterfall(/*ID3D11Device*/ void* device, UINT* indices)
{
	m_vertexCount = (UINT)(*modelTextureWaterfall).size();
	DXTextureTextureWaterfallType* vertices = NEW DXTextureTextureWaterfallType[m_vertexCount];	// Create the vertex array.
	IF_NOT_THROW_EXCEPTION(vertices);
	...
	{
		// Load the vertex array with data.
		vertices[i].position = XMFLOAT3((*modelTextureWaterfall)[i].x, (*modelTextureWaterfall)[i].y, (*modelTextureWaterfall)[i].z);
		vertices[i].texCoord = XMFLOAT2((*modelTextureWaterfall)[i].tu, (*modelTextureWaterfall)[i].tv);
		vertices[i].color = XMFLOAT4((*modelTextureWaterfall)[i].r, (*modelTextureWaterfall)[i].g, (*modelTextureWaterfall)[i].b, (*modelTextureWaterfall)[i].a);
	#if defined USE_BOUNDING_VOLUMES
		CALCULATE_MAX_MIN(vertices[i].position);
	#endif
	}

	IF_NOT_RETURN_FALSE(CreateDXbuffers(sizeof(DXTextureTextureWaterfallType), device, indices, vertices));
...
bool DXmodelClass::InitializeTextureSmoke(/*ID3D11Device*/ void* device, void* indices)
{
	m_vertexCount = (UINT)(*modelSmoke).size();
	DXtextureVertexType* vertices = NEW DXtextureVertexType[m_vertexCount];	// Create the vertex array.
	IF_NOT_THROW_EXCEPTION(vertices);
	...
	//if (Model3D)
	{
		for (UINT i = 0; i < m_vertexCount; i++)
		{
			...
			vertices[i].position = XMFLOAT3(
				...
				(*modelSmoke)[i].y,
				(*modelSmoke)[i].z);
			vertices[i].texCoord = XMFLOAT2((*modelSmoke)[i].tu, (*modelSmoke)[i].tv);

#if true //defined _DEBUG && DX_ENGINE_LEVEL < 29
			womalog("vertices: %d %d %d - %f %f \n",
...
#if defined GENERATE_ATLAS_INTEGRATION_DDS
		case SHADER_BILLBOARD_ATLAS_FAST:
			stride[0] = sizeof(DXbillboardAtlasVertexType);
			break;
#endif
#if defined MAIN_RENDER_WATER_FALL
		case SHADER_USE_WATERFALL:
			stride[0] = sizeof(DXTextureTextureWaterfallType);
			break;
		case SHADER_USE_SMOKE:
			stride[0] = sizeof(DXtextureVertexType);
			break;
#endif
    ...
    for(UINT i = 0; i < obj3d.meshSubsets; ++i)
    {
		...
		if ((UINT)obj3d.material[obj3d.subsetMaterialArray[i]].transparent == pass || pass == PASS_SHADOWS)
	    {
			...
			else 
			{
				...
				if (ModelShaderType == SHADER_TEXTURE_LIGHT_FAST)
				useShader->SetFASTShaderParameters(pass, pContext, m_world, viewRender, projectionMatrix);// UPDATE CONST. BUFFER
				else
		...
		{
			{
				...
				if (ModelShaderType == SHADER_BILLBOARD_ATLAS_FAST)
				{
					ID3D11DeviceContext* deviceContext11 = (ID3D11DeviceContext*)pContext;

					// Bind the atlas texture SRV at slot t0
					if (billboardAtlasSRV)
						deviceContext11->PSSetShaderResources(0, 1, &billboardAtlasSRV);
				}
				else
				#endif
				...
				if (m_Shader11->m_shaderType == SHADER_Terrain_Texture_DEMO99)
				{
					pContext->PSSetShaderResources(15, 1, &SystemHandle->m_Application->m_TextureWithShadows->m_shaderTextureResourceView);			// set current alfaMap
					m_Shader11->SetShaderParameters(pass, pContext, &m_worldMatrix, viewMatrix, projectionMatrix, (XMMATRIX*)lightViewMatrix, (XMMATRIX*)ShadowProjectionMatrix);	// Set the shader parameters that it will use for rendering

					m_Shader11->RenderShader(pass, pContext, /*texture_index*/ 0, m_indexCount);					// Now render the prepared buffers with the shader
					return;
				}
			}
            ...
            #else
			  if (ModelShaderType == SHADER_USE_SMOKE)
				m_Shader11->RenderSmoke(pass, pContext, m_indexCount, &m_worldMatrix, viewMatrix, projectionMatrix, g_particleAlpha);
			  else
...
bool DirectX::DXmodelClass::LoadModel(void* pContext, TCHAR* objectName, void* g_driver, SHADER_TYPE shader_type, STRING filename, bool castShadow, bool renderShadow, UINT instanceCount)
{
    ...
    {
        {
			bool b = LoadW3DfromMEM(pContext, shader_type, g_driver, filename, castShadow, renderShadow, instanceCount);
            if (!b)
            {
