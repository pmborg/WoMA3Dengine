...
bool CTerrain::CreateTerrain60(ID3D11DeviceContext* pContext, float xPos, float zPos) 
{
	...
	CREATE_MODEL_IF_NOT_EXCEPTION(SystemHandle->m_Application->m_TerrainModel[MINIMAP_TERRAIN_ID], I_AM_3D, I_HAVE_NO_SHADOWS, I_HAVE_NO_SHADOWS);  // m_TerrainModel[id] = NEW
	SystemHandle->m_Application->m_TerrainModel[MINIMAP_TERRAIN_ID]->ModelHASfog = true;

	ASSERT(SystemHandle->m_Application->m_TerrainModel[MAIN_TERRAIN_ID]->LoadTerrain(pContext, TEXT("Terrain 99"), m_Driver, SHADER_Terrain_Texture_DEMO99, &Terrain60Textures, &modelVertexVector2, &Terrain60indices));
	ASSERT(SystemHandle->m_Application->m_TerrainModel[MINIMAP_TERRAIN_ID]->LoadTerrain(pContext, TEXT("MiniMap-Terrain"), m_Driver, SHADER_Terrain_Texture_DEMO61, &Terrain60Textures, &modelVertexVector2, &Terrain60indices));
