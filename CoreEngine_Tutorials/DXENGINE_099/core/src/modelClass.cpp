...
// Populate: obj3d
// --------------------------------------------------------------------------------------------
bool ModelClass::LoadOBJ(void* ctx, void* dxmodelClass, SHADER_TYPE shader_type, void* g_driver, STRING filename, bool castShadow, bool renderShadow, UINT instanceCount, UINT instanceType)
// --------------------------------------------------------------------------------------------
{	HRESULT hr = 0;
							...
							else if(checkChar == 'd')
							{
							  ...
							  if (WOMA::AppSettings->DRIVER == DRIVER_DX11 || WOMA::AppSettings->DRIVER == DRIVER_DX9) 
							  {
								...
								if (WOMA::AppSettings->DRIVER == DRIVER_DX12)
								{
									...
									bool result = alfaMap->Initialize(m_driver, (TCHAR*)fileNamePath.c_str(), 0, /*Model3D*/ TRUE);	// Initialize the texture object:
									...
									if (result)
									{
										...
										// |2| DescriptorTable  | t0				| Alfa Map
										// |3| DescriptorTable  | t1				|

										//Prepare Teture to be uploaded from CPU/MEMORY ------> gpu
										CD3DX12_CPU_DESCRIPTOR_HANDLE cbvHandle1(((DXmodelClass*)dxmodelClass)->m_Shader->DX12mSrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 
																				 ((DXmodelClass*)dxmodelClass)->m_Shader->m_CbvSrvDescriptorSize, 2); // AlfaMap
										m_driver->m_device->CreateShaderResourceView(alfaMap->m_pTexture.Get(), &m_driver->DX12viewDesc, cbvHandle1);
									}
								}
							  #endif
                            #endif
							...
							if(checkChar == 't')
							{
								...
								if(checkChar == 'l')
								{
									...
									if(checkChar == ' ')
									{
									  ...
									  #endif
									  #if defined DX12
										obj3d.material[matCount].alfaMap = NULL;
									  #endif
									#endif
...
void ModelClass::CalculateTangentBinormal(ModelNormalBumpVertexType vertex1, ModelNormalBumpVertexType vertex2, ModelNormalBumpVertexType vertex3, VectorType& tangent, VectorType& binormal)
{
								...
								void* g_driver, SHADER_TYPE shader_type, STRING filename, bool castShadow, bool renderShadow)
{
...
#endif
				if (obj3d.meshMaterials[i].size() > 0)
	...
	if (obj3d.ModelHASNormals) // Forced to be TRUE on CH51!
	{
		...
		{
			//1
			tempVert1.x = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j]]].x;
			tempVert1.y = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j]]].y;
			tempVert1.z = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j]]].z;

			tempVert1.tu = obj3d.vertTexCoord[obj3d.vertTCIndex[obj3d.indices32[j]]].x;
			tempVert1.tv = obj3d.vertTexCoord[obj3d.vertTCIndex[obj3d.indices32[j]]].y;

			if (obj3d.vertNormIndex[obj3d.indices32[j]] < (int)obj3d.vertNorm.size())				//Make it more rubust
			{
				tempVert1.nx = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j]]].x;
				tempVert1.ny = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j]]].y;
				tempVert1.nz = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j]]].z;
			}

			//if (j + 1 < obj3d.m_vertexCount) 
			{
				//2
				tempVert2.x = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j+1]]].x;
				tempVert2.y = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j+1]]].y;
				tempVert2.z = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j+1]]].z;

				tempVert2.tu = obj3d.vertTexCoord[obj3d.vertTCIndex[obj3d.indices32[j+1]]].x;
				tempVert2.tv = obj3d.vertTexCoord[obj3d.vertTCIndex[obj3d.indices32[j+1]]].y;

				if (obj3d.vertNormIndex[obj3d.indices32[j+1]] < (int)obj3d.vertNorm.size())				//Make it more rubust
				{
					tempVert2.nx = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j+1]]].x;
					tempVert2.ny = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j+1]]].y;
					tempVert2.nz = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j+1]]].z;
				}
			}

			//if (obj3d.indices32[j+2] < obj3d.m_vertexCount)
			{
				//3
				tempVert3.x = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j+2]]].x;
				tempVert3.y = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j+2]]].y;
				tempVert3.z = obj3d.vertPos[obj3d.vertPosIndex[obj3d.indices32[j+2]]].z;

				tempVert3.tu = obj3d.vertTexCoord[obj3d.vertTCIndex[obj3d.indices32[j+2]]].x;
				tempVert3.tv = obj3d.vertTexCoord[obj3d.vertTCIndex[obj3d.indices32[j+2]]].y;

				if (obj3d.vertNormIndex[obj3d.indices32[j+2]] < (int)obj3d.vertNorm.size())				//Make it more rubust
				{
					tempVert3.nx = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j+2]]].x;
					tempVert3.ny = obj3d.vertNorm[obj3d.vertNormIndex[obj3d.indices32[j+2]]].y;
			...
			//1
			tempVert1.tx = tangent.x;
			tempVert1.ty = tangent.y;
			tempVert1.tz = tangent.z;
			tempVert1.bx = binormal.x;
			tempVert1.by = binormal.y;
			tempVert1.bz = binormal.z;

			//2
			tempVert2.tx = tangent.x;
			tempVert2.ty = tangent.y;
			tempVert2.tz = tangent.z;
			tempVert2.bx = binormal.x;
			tempVert2.by = binormal.y;
			tempVert2.bz = binormal.z;

			//3
			tempVert3.tx = tangent.x;
			tempVert3.ty = tangent.y;
			tempVert3.tz = tangent.z;
			tempVert3.bx = binormal.x;
			tempVert3.by = binormal.y;
			tempVert3.bz = binormal.z;
...
*/
		...
		if (WOMA::AppSettings->DRIVER != DRIVER_GL3)
			...
			((GLmodelClass*)XmodelClass)->LoadBump(pContext, (TCHAR*)filename.c_str(), g_driver, /*shader_type*/SHADER_NORMAL_BUMP, &obj3d.textureNameArray, &vertices, &obj3d.indices32);
	#endif
		...
		if ((obj3d.hasNorm || shader_type == SHADER_TEXTURE_LIGHT || shader_type == SHADER_TEXTURE_LIGHT98 || shader_type == SHADER_BILLBOARD_ATLAS_FAST) && shader_type != SHADER_FIRE || shader_type == SHADER_TEXTURE_GS_INSTANCED)
		{
			...
			{
				std::vector<ModelBillboardAtlasVertexType> modelBillboardVertex;
				ModelBillboardAtlasVertexType tempVert;
				...
				for (UINT j = 0; j < obj3d.m_vertexCount; ++j)
				{
					...
					if (obj3d.vertNormIndex[j] < (int)obj3d.vertNorm.size())
					{
						tempVert.nx = obj3d.vertNorm[obj3d.vertNormIndex[j]].x;
						tempVert.ny = obj3d.vertNorm[obj3d.vertNormIndex[j]].y;
						tempVert.nz = obj3d.vertNorm[obj3d.vertNormIndex[j]].z;
					...
					UINT treeid_ = ((DXmodelClass*)XmodelClass)->xmlId - SystemHandle->m_Application->world_xml_objs;
					UINT type = m_Trees[treeid_].type;
					...
					tempVert.ox = m_Trees[treeid_].vPos.x; //SystemHandle->xml_loader.theWorldXML[id_].posX;
					tempVert.oy = m_Trees[treeid_].vPos.y; //SystemHandle->xml_loader.theWorldXML[id_].translateY;
					tempVert.oz = m_Trees[treeid_].vPos.z; //SystemHandle->xml_loader.theWorldXML[id_].posZ;

					tempVert.scale = m_Trees[treeid_].scale; //SystemHandle->xml_loader.theWorldXML[id_].scale;
					tempVert.rotY = m_Trees[treeid_].rotY;   //SystemHandle->xml_loader.theWorldXML[id_].rotY;

					modelBillboardVertex.push_back(tempVert);
				}

				if (WOMA::AppSettings->DRIVER != DRIVER_GL3)
				{
					((DXmodelClass*)XmodelClass)->LoadBillboardAtlas(
						(ID3D11DeviceContext*)pContext,
						(TCHAR*)filename.c_str(),
						g_driver,
						SHADER_BILLBOARD_ATLAS_FAST,
						NULL,
						&modelBillboardVertex,
						&obj3d.indices32
					);
				}
			}
			...
			if (obj3d.hasTexCoord)
			{
				...
				if (WOMA::AppSettings->DRIVER != DRIVER_GL3)
					...
					((GLmodelClass*)XmodelClass)->LoadTexture(pContext, (TCHAR*)filename.c_str(), g_driver, /*shader_type*/ SHADER_TEXTURE, &obj3d.textureNameArray, &modelTextureVertex, &obj3d.indices32);
				#endif
			}
			else
			// SHADER_COLOR - 30
			{
				...
				if (WOMA::AppSettings->DRIVER != DRIVER_GL3)
					...
					((GLmodelClass*)XmodelClass)->LoadColor(pContext, (TCHAR*)filename.c_str(), g_driver, /*shader_type*/ SHADER_COLOR, &modelColorVertex, &obj3d.indices32);
			#endif
			}
