...
public:
	...
	bool LoadWaterFall(void* pContext,
		TCHAR* objectName, void* driver,
		SHADER_TYPE shader_type,
		std::vector<STRING>* textureFile, std::vector<ModelTextureWaterfall>* model,
		std::vector<UINT>* indexList, UINT instanceCount = 0);
	bool LoadSmoke(void* pContext,
		TCHAR* objectName, void* driver,
		SHADER_TYPE shader_type,
    ...
    bool bottleHit=false;			// Means this player is selected to a detail view...
#endif
	// MODEL LOAD:
	ADVOBJ3D obj3d;
...
// DX Specific:
...
#if defined GENERATE_ATLAS_INTEGRATION_DDS
	bool LoadBillboardAtlas(void* pContext, TCHAR* objectName, void* driver,
		SHADER_TYPE shader_type, std::vector<STRING>* textureFile,
		std::vector<ModelBillboardAtlasVertexType>* model, std::vector<UINT>* indexList, UINT instanceCount=0);

	bool InitializeBillboardAtlasBuffers(ID3D11Device* device, const std::vector<AtlasRegion>& regions);

	bool InitializeBillboardAtlasBuffers(void* device, void* indices);
#endif

	std::vector<ModelBillboardAtlasVertexType>* modelBillboardAtlasVertex = NULL;	// MODEL!

	// Shader resource view for the big billboard atlas texture
	//ID3D11ShaderResourceView* billboardAtlasSRV = nullptr;

	// Shader resource view for the structured buffer containing UV offsets/scales
	ID3D11ShaderResourceView* billboardAtlasRegionsSRV = nullptr;
	...
	std::vector<ModelShadowMapVertexType>* modelShadowMapVertex = NULL;	// MODEL!
	std::vector<ModelShadowMapVertexType> modelShadowMapVertex_;		// LOAD W3D
	bool InitializeShadowMapBuffers(void* g_driver, UINT* indices);
#endif
	...
	std::vector<ModelTextureWaterfall>* modelTextureWaterfall = NULL;
	std::vector<SmokeVertexType>* modelSmoke = NULL;
	bool InitializeTextureWaterfall(void* g_driver, UINT* indices);
	bool InitializeTextureSmoke(void* device, void* indices);
...
#if DX_ENGINE_LEVEL >= 60 && defined USE_TERRAIN_TUTORIAL_CHAP_24
	std::vector<HeightMapType_24>* modelTextureHeightMapType_24_Terrain=NULL;
	bool InitializeTextureHeightMapType_24_Terrain(void* g_driver, UINT* indices);
#endif
