...
DX9Class::DX9Class() 
{
	...
	if (!WOMA::UseWarpDevice)
		_tcscpy_s(driverName, sizeof(driverName), TEXT("DX9"));
	else
		_tcscpy_s(driverName, sizeof(driverName), TEXT("DX9 WARP"));
	...
	if (m_device)
	{
		...
		if (!RenderfirstTime)
		{
			...
			if (hr != D3D_OK)
			{
				...
				WomaMessageBox(TEXT("Call to Reset() failed!"));
				return false;
			}
		}
...
void DX9Class::SetCamera2D()
{
	...
	m_Camera->Use2DViewMatrix();	// SAVE the 2D Calculated View Matrix: (m_viewmatrix2D = m_viewMatrix;)
}
...
// ----------------------------------------------------------------------------------------------
{
	...
	{
		WomaMessageBox(TEXT("GetAdapterDisplayMode NOK"));
		return FALSE;
	}
	...
	*/
	...
	{
		WomaMessageBox(TEXT("Directx Device: GetDeviceCaps")); return FALSE;
	}
	...
	{
		{WomaMessageBox(TEXT("This Graphics Card dont Support the basic Vertix Shader")); return false; }
	}
	...
	{
		g_MSAA_SUPPORT = true;
		WOMA::logManager->DEBUG_MSG(TEXT("DRIVER multiSampleQuality: %d\n"), QualityLevels);	// Get the max MsaaQuality: 32
	} else {
		g_MSAA_SUPPORT = false;
		WomaMessageBox(TEXT("WARNING: This card don't support, Multisample anti-aliasing")); // NOTE: Don't make it fatal (just reset setting)
	}
...
// ----------------------------------------------------------------------------------------------
{
		...
		if (m_CameraMINIMAP)
		{
			delete ((DirectX::DXcameraClass*)m_CameraMINIMAP);
			m_CameraMINIMAP = NULL;
		}
