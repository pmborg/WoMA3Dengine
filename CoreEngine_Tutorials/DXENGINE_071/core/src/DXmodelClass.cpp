...
bool DXmodelClass::LoadTerrain(	TCHAR* objectName, 
								...
								std::vector<ModelTextureDouble_Color_TerrainType> *model, std::vector<UINT>* indexList, 
								UINT instanceCount)
{
	LOADDRIVER(driver);
	MODEL_NAME = objectName;
	...
	UINT instanceCount)
{
	...
	//	ModelShaderType = SHADER_Terrain_Texture_DEMO60;
	//else
	ModelShaderType = shader_type;
...
bool DXmodelClass::InitializeDXbuffers(TCHAR* objectName, std::vector<STRING>* textureFile)
{
	...
	case SHADER_Double_Color_Terrain:
	case SHADER_Slope_Texture_Terrain:
		result = InitializeTextureDouble_Color_Terrain(m_driver11->m_device11, indices); break;
	case SHADER_Terrain_Texture_DEMO19:
		result = InitializeTextureDouble_Color_Terrain_TexMapping(m_driver11->m_device11, indices); break;
#if DX_ENGINE_LEVEL >= 60 && defined USE_TERRAIN_TUTORIAL_CHAP_24
	case SHADER_Terrain_Texture_DEMO60:
	case SHADER_Terrain_Texture_DEMO61:
		result = InitializeTextureHeightMapType_24_Terrain(m_driver11->m_device11, indices); break;
#endif
...
#if DX_ENGINE_LEVEL >= 38 && !defined INTRO_DEMO
	if (Model3D && obj3d.fileNameOnly.length() > 0)
...
bool DXmodelClass::LoadModel(TCHAR* objectName, void* g_driver, SHADER_TYPE shader_type, STRING filename, bool castShadow, bool renderShadow, UINT instanceCount)
{
	...
	else if (_tcsicmp(extension, TEXT(".md5mesh")) == 0 || _tcsicmp(extension, TEXT(".MD5MESH")) == 0) {
		bool b = modelClass.LoadMD5(this, shader_type, g_driver, filename, castShadow, renderShadow, instanceCount);
	}
#endif
