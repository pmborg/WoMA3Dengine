...
void InitMeshDemo(ID3D11DeviceContext* pContext, ApplicationClass* app, MeshApplication* demoapp, MyDemo* demo)
{
    ...
    womamesh4[MAIN_CHAR_MODEL2].scene.Start(pContext, demoapp->m_Graphics);
    womamesh4[MAIN_CHAR_MODEL3].scene.Start(pContext, demoapp->m_Graphics);
}
...
void UpdateAllMeshAnimations(float deltaTime)
{
    ...
    if (womamesh3.assimpSceneModel && womamesh3.assimpSceneModel->loaded)
    {
        ...
        womamesh3.assimpSceneModel->readyToRender = true;
    }
#endif
...
void RenderAllMeshModels(ID3D11DeviceContext* m_DeviceContext, float fadeLight=1)
{
...
#if DX_ENGINE_LEVEL >= 86 && defined USE_MODEL4
    if (womamesh4[MAIN_CHAR_MODEL1].assimpSceneModel &&
        womamesh4[MAIN_CHAR_MODEL1].assimpSceneModel->readyToRender)
    // Model 4 ------------------------------------------------------------------------------------------
    {
        XMMATRIX world = XMMatrixIdentity();
        //Scale:
...
#if DX_ENGINE_LEVEL == 87 && defined USE_MODEL4
    if (womamesh4[MAIN_CHAR_MODEL2].assimpSceneModel &&
        womamesh4[MAIN_CHAR_MODEL2].assimpSceneModel->readyToRender)
    // Model 4 ------------------------------------------------------------------------------------------
    {
        XMMATRIX world = XMMatrixIdentity();
        //Scale:
    ...
    // Model 4 ------------------------------------------------------------------------------------------
    if (womamesh4[MAIN_CHAR_MODEL3].assimpSceneModel &&
        womamesh4[MAIN_CHAR_MODEL3].assimpSceneModel->readyToRender)
    {
        XMMATRIX world = XMMatrixIdentity();
        //Scale:
        //_11
