...
void DX11Class::InspectDeviceCapabilities(D3D_FEATURE_LEVEL featureLevel)
{
	...
	if (!FAILED(result))
	{
		...
		womalog(TEXT("DXGI1.1: Available\n"));
	}
	...
	if (!FAILED(result))
	{
		...
		womalog(TEXT("DXGI1.2: Available\n"));
	}
	SAFE_RELEASE(dxgiDevice2);

	womalog(TEXT("DEVICE_CAPABILITIES:\n"));
