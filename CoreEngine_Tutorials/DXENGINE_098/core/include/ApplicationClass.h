...
struct MESH 
{
	...
	static std::vector<Filme> saveFilme; //SAVE_WALK

	std::vector<Filme> loadFilme;
	PlayerClass* m_character = NULL;
	PositionClass* m_characterPos = NULL;
};

extern MESH demo_mesh1;
#endif

#if defined USE_3RD_PERSON_CAMERA || defined USE_DEMO99
struct MAIN_3RD_PERSON_CAMERA
{
	float m_camYaw = 0.0f;
...
extern MAIN_3RD_PERSON_CAMERA main3rdPcamera;
#endif
...
public:
	...
	VirtualModelClass* model_waterfall = NULL;
	VirtualModelClass* m_smokeModel = NULL;
	// VARS:
	// ----------------------------------------------------------------
...
//83&84:
    ...
    void RenderMiniMapPass(UINT monitorWindow, WomaDriverClass* Driver, void* pContext, float fadeLight, UINT ThreadID=0);
    void RenderMainMapMiniMap(void* pContext);
...
public:
	...
	void CheckEditor(int hitIndex, int c);
#endif

#if defined USE_MINIMAP_REDENRING_THREAD
	void CreateRenderThreads();
	void StopRenderThreads();
	void WaitForPreRenderTasks(void* mainCtx);
#if defined USE_UPDATEMESHANIMATIONS_THREAD
	void UpdateMeshAnimationsWorkerFunc();
#endif
	void MinimapWorkerFunc();
#endif
};
