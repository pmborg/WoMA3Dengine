...
#if DX_ENGINE_LEVEL >= 94 && defined USE_TREE_POINTERV2
...
)
{
	...
	((DXmodelClass*)AtlasobjModel)->m_indexCount = outIdxCount; // idx.size();
}
...
void ApplicationClass::SortOutWhatNeedToBeRendered(void* pContext, WomaDriverClass* driver)
{
	...
	std::sort(WOMA::sceneManager->visibleBillboardList.begin(), WOMA::sceneManager->visibleBillboardList.end(),
		[](const Tree* a, const Tree* b) {
			return a->sortKey > b->sortKey; // Farther first
		});
#elif DX_ENGINE_LEVEL >= 94 && defined USE_TREE_POINTERV2
	std::sort(WOMA::sceneManager->visibleBillboardList.begin(), WOMA::sceneManager->visibleBillboardList.end(), BillSortCB_CPP_key);
#else
...
void ApplicationClass::RenderMiniMapPass(UINT monitorWindow, WomaDriverClass* Driver, void* pContext, float fadeLight, UINT ThreadID)
{
...
#if DX_ENGINE_LEVEL >= 63 && defined USE_MINI_MAP
	{
			...
			//for (int id = world_main_size - 1; id >= 0; --id) 
		{
			...
			RenderModel(pContext, 1, monitorWindow, m_Driver, id, PASS_OPAC, &m_CameraMINIMAP.m_viewMatrix, &((DirectX::DX11Class*)Driver)->m_projectionMiniMapMatrix, fadeLight);
			m_RenderMapTexture->SetRenderTarget(Driver, (ID3D11DeviceContext*)pContext);						// Set the render target to be the render to texture.
			RenderModel(pContext, 1, monitorWindow, m_Driver, id, PASS_OPAC, &m_CameraMAP.m_viewMatrix, &((DirectX::DX11Class*)Driver)->m_projectionMiniMapMatrix, fadeLight);
		}
#endif
...
void ApplicationClass::AppRender(UINT monitorIndex, float fadeLight, void* pContext)
{
	...
	for (UINT id = 0; id < WOMA::sceneManager->visibleModelList.size(); id++)
	{
		...
		{
			case 401:
				//((DXmodelClass*)WOMA::sceneManager->visibleModelList[id])->m_worldMatrix = XMMatrixIdentity();
				RenderModel(pContext, 0, monitorIndex, m_Driver, id, PASS_OPAC, NULL, NULL, fadeLight, 0);
				RenderModel(pContext, 0, monitorIndex, m_Driver, id, PASS_OPAC, NULL, NULL, fadeLight, 1);
				//objModel[id]->Render(pContext, 0, CAMERA_NORMAL, PROJECTION_PERSPECTIVE, PASS_OPAC, NULL, NULL);
				break;
				default:
