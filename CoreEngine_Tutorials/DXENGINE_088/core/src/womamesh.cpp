...
#endif
    womamesh4[MAIN_CHAR_MODEL1].assimpSceneModel = SceneModel::LoadModelToScene(DX_ENGINE_LEVEL, false, 1, WOMA::LoadFile((TCHAR*)TEXT("engine/data/scene87ForestHuntress/")), "", womamesh4[MAIN_CHAR_MODEL1].scene, demoapp->m_Graphics);
...
void RenderMesh(MeshApplication* demoapp, MyDemo* demo)
{
    ...
    // Model 4 ------------------------------------------------------------------------------------------
    {
        ...
        womamesh4[USING_CHAR_MODEL_ANIMATION].scene.UpdateModel(demoapp->m_Graphics, world);
    }
    demo->gBufferPass->Render(demoapp->m_Graphics, womamesh4[USING_CHAR_MODEL_ANIMATION].scene);
}
