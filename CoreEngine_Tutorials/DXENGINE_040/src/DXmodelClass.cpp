...
DXshaderClass* DXmodelClass::CreateShader(TCHAR* objectName, SHADER_TYPE ShaderType)
{
	...
	if	(ShaderType == SHADER_TEXTURE_LIGHT_INSTANCED ||
		ShaderType == SHADER_TEXTURE_LIGHT_CASTSHADOW_INSTANCED ||
		ShaderType == SHADER_NORMAL_BUMP_INSTANCED)
...
bool DXmodelClass::InitializeDXbuffers(TCHAR* objectName, std::vector<STRING>* textureFile)
{
	...
	if (m_instanceCount > 0) //1 or more can be Instancing: its not an error, DONT TOUCH THIS!
	{
		if (ModelShaderType == SHADER_TEXTURE_LIGHT)
			ModelShaderType = SHADER_TEXTURE_LIGHT_INSTANCED;			// Switch SHADER_TEXTURE_LIGHT ==> SHADER_TEXTURE_LIGHT_INSTANCED
		else
		if (ModelShaderType == SHADER_TEXTURE_LIGHT_CASTSHADOW)
			ModelShaderType = SHADER_TEXTURE_LIGHT_CASTSHADOW_INSTANCED;// Switch SHADER_TEXTURE_LIGHT_CASTSHADOW ==> SHADER_TEXTURE_LIGHT_CASTSHADOW_INSTANCED
		else
		if (ModelShaderType == SHADER_NORMAL_BUMP)
			ModelShaderType = SHADER_NORMAL_BUMP_INSTANCED;				// Switch SHADER_NORMAL_BUMP ==> SHADER_NORMAL_BUMP_INSTANCED
	}
...
// ----------------------------------------------------------------------------------------
{
	...
	if (Model3D && obj3d.fileNameOnly.length() > 0)
		SaveM3D(obj3d.fileNameOnly, indices, vertices, sizeofMODELvertex);
