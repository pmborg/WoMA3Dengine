...
void UpdateAllMeshAnimations(float deltaTime)
{
        ...
        womamesh4[USING_CHAR_MODEL_ANIMATION].assimpSceneModel->loaded)
    {
        ...
        womamesh4[USING_CHAR_MODEL_ANIMATION].assimpSceneModel->readyToRender = true;
    }
}
...
void RenderAllMeshModels(ID3D11DeviceContext* m_DeviceContext, float fadeLight=1)
{
    ...
    // Model 4 ------------------------------------------------------------------------------------------
    if (womamesh4[USING_CHAR_MODEL_ANIMATION].assimpSceneModel && 
        womamesh4[USING_CHAR_MODEL_ANIMATION].assimpSceneModel->readyToRender)
    {
        XMMATRIX world = XMMatrixIdentity();
        //Scale:
        //_11
        ...
        demo->gBufferPass->Render(m_DeviceContext, demoapp->m_Graphics, womamesh4[USING_CHAR_MODEL_ANIMATION].scene, fadeLight);
    }
}
