...
bool DX11Class::InitD2D_D3D101_DWrite(IDXGIAdapter1 *Adapter, WCHAR* fontStyle, int screenWidth, int screenHeight, float R, float G, float B)
{
	...
	sharedTexDesc.SampleDesc.Quality = MSAA_QUALITY;	//Anti-Alising: Texture Filtering tech. (MSAA > 0)
	...
	float FontSizeIndex = 22; // MAX(24, 24.0f * MIN(1, 2 * SystemHandle->m_scaleX)); // At least 8, Max: 24 (with this formula Text Scale Better)
